    class TimePicker : DomainUpDown
    {
        public TimePicker()
        {
            ...
            for (double time = 23.5; time >= 0; time -= 0.5)
            {
                ...
                times.Add(hour.ToString("00") + ":" + minutes.ToString("00")); // format the hour and minutes to always have two digits and concatenate them together with the colon between them
            }
            this.Items.AddRange(times); // add the times to the Items collection
            this.SelectedIndex = times.IndexOf("09:00"); // select a default time
