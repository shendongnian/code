    ...
    namespace stackoverflow.fv
    {
        ...
        public class MyValidator : NamedMessageAbstractValidator<My>
        {
            public MyValidator()
            {
                RuleFor(x => x.Name).NotEmpty()
                    .WithMessage("{0}", x => CreateNamedMessage("The name {Name} is not valid for Id {Id}", x));
            }
        }
        public abstract class NamedMessageAbstractValidator<T> : AbstractValidator<T>
        {
            protected string CreateNamedMessage(string format, T target)
            {
                return format.JamesFormat(target);
            }
        }
        public class My
        {
