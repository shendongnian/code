        ...
        public static DataTable LeftOuterJoinTwoDataTablesOnOneColumn(DataTable dtblWithUniqueValuesInColumn, DataTable dtblWithExtraData, string colToJoinOn)
        {
            ...
            //Merge extra data into the result table. This is the table that does not have unique keys.
            dtblResult.Merge(dtblWithExtraData, false, MissingSchemaAction.AddWithKey);
            //Add the unique row data to the result table and do it by column name.
            foreach (DataRow drow in dtblResult.Rows)
            {
                string key = drow[colToJoinOn].ToString();
                DataRow[] drows = dtblWithUniqueValuesInColumn.Select("Convert([" + colToJoinOn + "],'System.String')='" + key + "'", "", DataViewRowState.CurrentRows);
                if (drows.Length > 0)
                {
                    foreach (DataColumn dcol in drows[0].Table.Columns)
                    {
                        if (drow.Table.Columns.Contains(dcol.ColumnName))
                        {
                            drow[dcol.ColumnName] = drows[0][dcol.ColumnName];
                        }
                    }
                }
            }
