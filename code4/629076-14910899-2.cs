    ...
    foreach (ListItem item in chkGeneralSkills.Items)
    {
    	if (item.selected)
    	{       
                // if there are no items in the list, omit the OR
    		if (!selectedItems.Any())
    			selectedItems.Add(" sl.SkillTitle = " + item.text)
    		else // otherwise, add the OR
    			selectedItems.Add(" OR sl.SkillTitle = " + item.text)
    	}
    }
    ...
    using (SqlConnection oConn = new SqlConnection(ConfigurationManager.ConnectionStrings["connString"].ToString()))
    {
        string sql = "SELECT us.FName + ' ' + us.SName As 'Name', " +
                    ...
                    "LEFT JOIN SkillsListing sl ON sl.SkillsListingID = sk.SkillsListingID " + string.Join(" ", selectedItems.ToArray()); // join together the items in the string list to make the where clause for Sql Server
        try
        {
            oConn.Open();
