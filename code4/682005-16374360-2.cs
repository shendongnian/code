	static public T CreateConstructorDelegate<T>(this Type type)
	{
		...
		ConstructorInfo constructor = type.GetConstructor(BindingFlags.Instance | BindingFlags.Public, null, delegateParams.Skip(1).Select(p => p.ParameterType).ToArray(), null);
		if (constructor == null)
