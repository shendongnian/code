    ...
    // psEncrypted for powershell> convertfrom-securestring -SecureString $aSecureString -key (1..16)
    ...
    public static SecureString ConvertToSecureString(string psEncrypted, byte[] key)
    {
        ...
        {
            // using this silly loop format so we never store the entire password naked
            int numRed = 0;
    ...
    public static SecureString DecryptPassword( string psPasswordFile, byte[] key = null )
    {
