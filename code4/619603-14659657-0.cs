    ...
    {                                  
        ...
        while (sqlDataReader.Read())
        {
            for (int i = 0; i < sqlDataReader.FieldCount; i++)
            {
                ...
                textwriter.WriteLine(sqlDataReader.GetName(i), sqlDataReader[i].ToString());
                // perhaps you meant to write the DB column name and field value separately?
                //
                // textwriter.WriteLine(sqlDataReader.GetName(i));
                // textwriter.WriteLine(sqlDataReader[i].ToString());
            }
            textwriter.Flush();
        }
    }
