    public static class ExpressionExtensions
    {
        public static Expression<Func<T, bool>> AddContains<T>(this LambdaExpression selector, string value)
        {
			...
			var body = selector.GetBody().AsString(valueLength);
        ...
        public static Expression AsString(this Expression expression, int length)
        {
            if (expression.Type == typeof (string))
                return expression;
            MethodInfo toString = typeof(SqlFunctions).GetMethods().First(m => m.Name == "StringConvert" && m.GetParameters().Length == 2 && m.GetParameters()[0].ParameterType == typeof(double?));
            var cast = Expression.Convert(expression, typeof(double?));
            return Expression.Call(toString, cast, Expression.Constant(length, typeof(int?)));
        }
    }
