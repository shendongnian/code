	...
	namespace ScheduledWorker
	{
		...
		public partial class ScheduledWorker : Component, ISupportInitialize
		{
			...
			private readonly SendOrPostCallback workCallback;
			...
			private AsyncOperation workThreadOperation;
			...
			public ScheduledWorker(double interval) : base()
			{
				...
				workCallback = new SendOrPostCallback(AsynOperationCompleted);
				completedCallback = new SendOrPostCallback(OnFinished);
				progressCallback = new SendOrPostCallback(ProgressReporter);
				initializing = false;
				delayedEnable = false;
				workHandler = new EventHandler<DoScheduledWorkEventArgs>(WorkerThredStart);
			}
			/// <summary>
			...
			public void ReportProgress(int percentProgress, object userState)
			{
				...
				else
				{
					...
					{
						mainThreadContext.Post(progressCallback, args);
					}
					else
					{
			...
			private void MyTimerCallback(object state)
			{
				lock (statusChangeLockObject)
				{
					if (enabled && !isOccupied)
					{
						isOccupied = true;
						isWorking = true;
						mainThreadContext = mainThreadOperation.SynchronizationContext.CreateCopy();
						FILE_TIME fileTime = new FILE_TIME();
						SafeNativeMethods.GetSystemTimeAsFileTime(ref fileTime);
						DateTime time = DateTime.FromFileTime((long)((((ulong)fileTime.ftTimeHigh) << 32) | (((ulong)fileTime.ftTimeLow) & 0xffffffff)));
						workHandler.BeginInvoke(this, new DoScheduledWorkEventArgs(Argument, time), null, null);
					}
				}
			}
