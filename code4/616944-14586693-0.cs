    private static string ResolveHostnameLdap(string hostname)
    {
        ...
        if (File.Exists(tnsAdminPath))
        {
            ...
            foreach (string directoryServer in directoryServers)
            {
                ...
                try
                {
                    ...
                    var directorySearcher = new DirectorySearcher(directoryEntry, "(&(objectclass=orclNetService)(cn=" + hostname + "))", new[] { "orclnetdescstring" }, SearchScope.Subtree);
