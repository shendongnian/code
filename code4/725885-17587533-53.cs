	For the digit products of consecutive numbers, we would met
	> 1 zero per 10
	>
	> 10 consecutive zeros per 100
	>
	> 100 consecutive zeros per 1000
	and so on. Note that there are still 9 zeros we would met with `per 10` in `per 100`. The count of zeros we would met can be calculated with the following code: 
	...
	**However**, with this implementation, we do **not** really skip the calculation of numbers contains zero, since I've found that the multiplication with zeros would **not** cause much impact of performance. I've tried another partial implementation which the zeros **do** matter(and so skipped): 
