                ...
                string chk_lock = "SELECT l.session_id,v.serial# ,object_name FROM dba_objects o, gv$locked_object l, v$session v WHERE o.object_id = l.object_id and l.SESSION_ID=v.sid";
             ...
             finally
                {
                        ...
                        while (rdr_blkAccount.Read())
                        {
                            if (rdr_blkAccount[2].ToString().ToUpper() == "ACCOUNT")
                            {
                                ...
                                serial = session_id.ToString() + ',' + Convert.ToInt32(rdr_blkAccount[1]).ToString();
                                kill_session = "alter system kill session '" + serial + "'";
                                result = db.ExecuteNonQuery(kill_session);
                                logger.Log(LogLevel.Warning, string.Format("Session_id '{0}' has been forcefully killed", serial));
                            }
                        }
