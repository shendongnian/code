	...
	class Recipe : Item
	{
		...
		public override IEnumerable<Item> ParseRecipe()
		{
			Console.WriteLine("1 " + this.Name + " = " + String.Join(" + ", this.Ingredients.Select(x => "1 " + x.Name)));
			var components = new List<Item>();
			foreach(var ing in Ingredients)
			{
				components.AddRange(ing.ParseRecipe());
			}
			if (Ingredients.Any(x => x.Ingredients != null))
			Console.WriteLine("1 " + this.Name + " = " + String.Join(" + ", components.Select(x => "1 " + x.Name)));
			return components;
		} 
	}
