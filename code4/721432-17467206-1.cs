    ...
    // to deal with user pasting text
     #region METHOD OnTextChanged
        ...
        protected override void OnTextChanged(EventArgs e)
        {
            ...
            // paste occured
            if (Math.Abs(currentLength - _prevTextLength) > 1)
            {
                if (this.NumericTextBoxType == NumericTextBoxType.TDecimal)
                {
                    NumberFormatInfo numberFormatInfo = System.Globalization.CultureInfo.CurrentCulture.NumberFormat;
                    string decimalSeparator = numberFormatInfo.NumberDecimalSeparator;
                    this.Text = Regex.Match(this.Text, (AllowMinus ? "-" : "") + @"?\d+(([.,]\d+)|(\d*))").Value;
                    this.Text = Regex.Replace(this.Text, @"[.,]", decimalSeparator);
                }
                else this.Text = Regex.Match(this.Text, (AllowMinus ? "-" : "") + @"?\d+").Value;
            }
            _prevTextLength = this.Text.Length;
        } 
        #endregion
        }
