            ...
            for (int i = 0; i < Environment.ProcessorCount; i++)
            {
                decompressors.Add(Task.Run(() =>
                {
                    using (ZipFile zipFile = new ZipFile(archivePath))
                    {
                        ...
                        {
                            // Report progress after each EntryBytesWritten event, as long as it's been at least 250ms since the last report, unless an extraction has completed,
                            // which we always want to report to ensure we don't finish without possibility updating progress to accurately show the completion.
                            if ((zipArgs.EventType == ZipProgressEventType.Extracting_EntryBytesWritten && progressReportingTimer.ElapsedMilliseconds >= 250) || zipArgs.EventType == ZipProgressEventType.Extracting_AfterExtractEntry)
                            {
