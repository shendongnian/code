    public abstract class AbstractAuthenticationController : Controller
    {
        ...
        protected Profile GetProfileForAuthenticatedUser()
        {
            var identityName = System.Web.HttpContext.Current.User.Identity.Name;
            var profile = _identityService.GetProfile(upn);
            if (profile == null) throw new ProfileNotFoundException("No Profile for " + identityName);
            return profile;
        }
    }
