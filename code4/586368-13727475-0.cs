            ...
            public ExcelDA()
        {
            connString = @"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=c:\sage.xls; Extended Properties=" + '"' + "Excel 8.0; HDR=Yes; IMEX=1" + '"';
        }
        /// <summary>
        ...
        public DataTable GetProductsExcel()
        {
            ...
            excBuilder.Append("SELECT * FROM [sage$];");
            //
            try
            {
        ...
        DataTable suppliers = reader.GetSuppliersExcel();
        //string array to store excel row data
        string[] arr2 = new string[2] { "", ""};
            //recursive loop to retrieve each row and values from each rows columns
            foreach (DataRow row in suppliers.Rows)
            {
                for (int i = 0; i < suppliers.Columns.Count; i++)
                {
                    if (i == (suppliers.Columns.Count - 1))
                    {
