    ...
    namespace ConsoleApplication2
    {
        internal class Program
        {
            ...
            {
                foreach (var permutation in Permute("Little Red Riding Hood".Split(' ')))
                {
            ...
            {
                int count = 0;  // We need to count the elements of the input sequence.
                ICollection collection = sequence as ICollection;
                if (collection != null) // Optimization - we have access to a '.Count' property.
                {
                    count = collection.Count;
                }
                else                    // No '.Count' property; have to count the elements via foreach.
                {
