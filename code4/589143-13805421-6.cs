	...
	[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
	public struct _WLAN_HOSTED_NETWORK_STATUS2
	{
		public _WLAN_HOSTED_NETWORK_STATE HostedNetworkState;
		public Guid IPDeviceID;
		[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 6)]
	...
	if (openHandleSuccess == 0)
	{
		...
		{
			var netStat2 = (_WLAN_HOSTED_NETWORK_STATUS2)Marshal.PtrToStructure(ptr, typeof(_WLAN_HOSTED_NETWORK_STATUS2));
			IntPtr offset = Marshal.OffsetOf(typeof(_WLAN_HOSTED_NETWORK_STATUS2), "PeerList");
			for (int i = 0; i < netStat2.dwNumberOfPeers; i++)
			{
