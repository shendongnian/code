    ...
    class Program
    {
        ...
        static void HandleFreeLimit(object sender, EventArgs e)
        {
            ...
            freeLimitArgs.FreeLimitReachedAction = (dynamic)Enum.Parse(gemboxAssembly.GetType("GemBox.Document.FreeLimitReachedAction"), "ContinueAsTrial");
        }
        ...
        static void Main(string[] args)
        {
            ...
            MethodInfo handleFreeLimitMethod = typeof(Program).GetMethod("HandleFreeLimit", BindingFlags.NonPublic | BindingFlags.Static);
    
            // Call: ComponentInfo.FreeLimitReached += HandleFreeLimit
            EventInfo freeLimitReached = componentInfo.GetEvent("FreeLimitReached");
            freeLimitReached.AddEventHandler(null, Delegate.CreateDelegate(freeLimitReached.EventHandlerType, handleFreeLimitMethod));
