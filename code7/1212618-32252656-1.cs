    // Change this empty list to load from dbo.EmailRecords into this list.
    List<EmailRecords> emailRecords = new List<EmailRecords>();
    ...
    // Set the new primary.
    EmailRecords emailRecordToSetToPrimary = emailRecords.Where(cs => cs.EmailRecordId == recordIdToChange).FirstOrDefault();
    ...
    List<EmailRecords> emailRecordsToUnsetFromPrimary = emailRecords.Where(cs => cs.EmailRecordId != recordIdToChange && cs.IsPrimary == true && cs.ParentRecordId == parentRecordIdToChange).ToList();
