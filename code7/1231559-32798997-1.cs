    public static T CallMultipleRecordSetStoredProcedure<T>(
        ...
        where T : class, new()
    {
        ...
        do
        {
            ...
            var dtoList = (IList) outputType.GetProperty(outputRecordSetPropertyName)
                .GetValue(output);
            ...
            while (reader.Read())
            {
                ...
                foreach (var property in propertiesToWrite)
                {
                    property.SetValue(
                        item,
                        Convert.ChangeType(
                            property.GetValue(reader[property.Name]),
