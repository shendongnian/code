    ...
    foreach (int workFlowServiceDetail in workFlowServiceDetails)
    {
       using (var db = new AdminDb())
       {
         ...
         var userListsTemp = PermissionCallMethod(classpath, workFlowServiceName, new[] { workFlowImplemented, eligibility }, serviceUrl);
    	...
    	if(userLists==null)
    	{
    		userLists=userListsTemp;
    	}
    	else
    	{
    		userLists.Concat(userListsTemp).ToList();	
    	}
