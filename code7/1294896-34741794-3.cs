    ...
    public void GetEntity_ValidName_EntityReturned()
    {
       ...
       mockEntityRepo .Setup(m => m.Find(It.IsAny<Predicate<Entity>>())).Returns(testEntity);
     // Or depending on type of Repo
      mockEntityRepo .Setup(m => m.Find(It.IsAny<Func<Entity, bool>>())).Returns(testEntity);
