     public IList<SearchHierarchyModel> GetHierarchyFull(int Continent_Id)
     {
         using (var context = new YourContext()) 
         {
             ...
             var countryList = context.Countries.Where(c => c.ContinentId == Continent_Id).ToList();
             //Get all cities that have a matching CountryId from any country in the first list.
             var cityList = context.Cities.Where(c => countryList.Any(cl => cl.Id == c.CountryId)).ToList();
