    public class IoCModelBinder : IModelBinder
    {
        ...
        {   // For reference: https://github.com/aspnet/Mvc/issues/4196
            ...
            if (bindingContext.Model == null && // This binder only constructs viewmodels, avoid infinite recursion.
                    (
                        (bindingContext.ModelType.Namespace == "OUR.SOLUTION.Web.ViewModels" && bindingContext.ModelType.Name.ToLower().Contains("viewmodel"))
                            ||
                        (bindingContext.ModelType.Namespace.Contains("OUR.SOLUTION.Core.Abstractions"))
                    )
                )
            {
