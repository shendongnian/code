    public async static void ScrollToIndex(this ListViewBase listViewBase, int index)
    {
        ...
        // when virtualized, scroll backward/forward a little bit (listViewBase.ActualHeight) to allow animation to be at least visible
        if (isVirtualizing)
        {
            if (isVerticalScrolling)
            {
                scrollViewer.ChangeView(null, position.Y + (position.Y > previousOffset ? -1 : 1) * listViewBase.ActualHeight, null, true);
            }
            else
            {
                scrollViewer.ChangeView(position.X + (position.X > previousOffset ? -1 : 1) * listViewBase.ActualWidth, null, null, true);
            }
        }
