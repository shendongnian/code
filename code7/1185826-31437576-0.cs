    Task TransferAndWaitStartAsync()
    {
        ...
        var whenStatusChanged = Observable.FromEventPattern<TransferStatusChangedEventArgs>(h, transferTask.StatusChanged += h, h => transferTask.StatusChanged -= h)
         ...
         // Continuation will complete when receiving the first event that match the predicate
         // in the observable sequence. Event if the event wast triggered too quickly.
