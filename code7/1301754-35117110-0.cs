        ...
        private async Task<bool> IsAuthorized(HttpContext context, ControllerBase controller)
        {
            ...
            var message = System.Text.Encoding.ASCII.GetString(Convert.FromBase64String(header));
            ...
            if (message[8] == '\x01')
            {
                ...
                context.Response.Headers.Add("WWW-Authenticate", "NTLM " + Convert.ToBase64String(type2Message));
                return false;
            }
            ...
            else if (message[8] == '\x03')
            {
                ...
                var user = controller.DbContext.Users.FirstOrDefault(u => u.WindowsAccount.ToLower() == userName.ToLower());
                if (user != null)
                {
