    ...
    namespace ConsoleApplication1
    {
        class Program
        {
            static void Main(string[] args)
            {
                ...
                string pattern = @"(?'title'.*)\.(?'year'[^\.]+)\.(?'size'[^\.]+)\.(?'format'[^\.]+)\.(?'filename'[^\.]+)\.(?'extension'[^\.]+)";
                foreach (string input in inputs)
                {
                    ...
                    title = title.Replace(".", " ");
                    Console.WriteLine("title = {0}, year = {1}, size = {2}, Format = {3}, filename = {4}.{5}",
                        title, year, size, format, fileName, extension);
                }
                Console.ReadLine();
            }
        }
