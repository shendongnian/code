    ...
    public class Player
    {
         ...
         public int GetTotalHandValue(IReadonlyList<Rule> rules)
         {
    ...
    public class Pot
    {
         ...
         public IReadonlyList<Player> GetWinners()
         {
              var playerHands = Enumerable.ToList(
                  ...
                  select new {
                      Player = player,
                      TotalHandValue = x.GetTotalHandValue()
                  }
              );
              var maxHand = playerHands.Max(x => x.TotalHandValue);
              return Enumerable.ToList(
                  from x in playerHands
                  where x.TotalHandValue == maxHand
                  select x.Player
              );
         }
    }
