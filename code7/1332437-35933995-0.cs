        ...
        var query = from item1 in _db.DailyJobCount
                    ...
                    group new { item1, item2 } by new { item1.Name, item1.Date, item1.NextDate } into subGroup
                    select new
                    {
                        subGroup.Key.Name,
                        subGroup.Key.Date,
                        subGroup.Key.NextDate,
                        Diff = SqlFunctions.DateDiff("day", subGroup.Key.Date, subGroup.Min(x => x.item2.Date))
                    };
