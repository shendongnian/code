    public partial class MainWindow : Window
    {
        ...
        public Color? CheckWinner()
        {
            ...
            {
                Color? lineResult = CheckWinnerForLine(0, row, Columns, 1, 0);
                if (lineResult != null)
                {
                    return lineResult;
                }
                if (Rows - row >= _kwinningCount)
                {
                    lineResult = CheckWinnerForLine(0, row, Rows - row, 1, 1);
                    if (lineResult != null)
                    {
                        return lineResult;
                    }
            ...
            {
                Color? lineResult = CheckWinnerForLine(column, 0, Rows, 0, 1);
                if (lineResult != null)
                {
                    return lineResult;
                }
                if (column > 0 && Columns - column >= _kwinningCount)
                {
                    lineResult = CheckWinnerForLine(column, 0, Columns - column, 1, 1);
                    if (lineResult != null)
                    {
                        return lineResult;
                    }
