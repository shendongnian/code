    ...
    {
        string[] exponentials = new string[] { "1.23E+4", "1.23E+04", "1.23e+4", "1.23", "1.23e+4e+4", "abce+def" };
    ...
    public static double? ParseExponential1(string input)
    {
        ...
        if (input.Contains("e+"))
        {
            string[] inputSplit = input.Split(new string[] { "e+" }, StringSplitOptions.None);
            if (inputSplit.Length == 2) // If there were not two elements split out, it's an invalid exponential.
            {
                ...
                {
                    return double.Parse(input); // If we got here, everything is in the right format and it is a good exponential. So no need for TryParse.
                }
            }
        }
        return null;
