    ...
    namespace WindowsFormsApplication
    {
        public partial class Form1 : Form
        {
            ...
            {
                InitializeComponent();
                myLine = new Line() { Start = new Point(x1, y1), Stop = new Point(x2, y2) };
            }
        ...
        public class Line
        {
            ...
            {
                // y = mx + c
                float m = (Stop.Y - Start.Y) / (Stop.X - Start.X); // m = dy/dx
                ...
                return p.X >= Math.Min(Start.X,Stop.X) 
                    ...
                    && (p.Y == (m*p.X)+c); // maybe you should add a wiggle room here since it's a float and line is very thin
            }
        }
    }
