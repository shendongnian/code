			...
			public void DeletingAndAddingSingleNovelists()
			{
				...
				using (var ctx = new DbContext())
				{
					...
					if (novelists != null && novelists.Any())
					{
						...
						if (war != null)
						{
							Debug.WriteLine(String.Format("'War and Peace' currenty has {0} {1}.",
								war.Novelists.Count, war.Novelists.Count == 1 ? "Author" : "Authors"));
							// assign the targeted novelists to 'War and Peace'
							novelists.ToList().ForEach(n => war.Novelists.Add(n));
							// save it all!
							ctx.SaveChanges();
						}
					}
