...
As pointed in many other answers on SO (like [that one](https://stackoverflow.com/a/54163263/4636721)), the `async` / `await` rewrite the code as a state machine just like for the `yield` statement with a method returning either `IEnumerator`, `IEnumerable`, `IEnumerator<T>`, `IEnumerable<T>`.
...
Basically in both cases of valid code containing `yield` or `async` keywords you have an initial state and depending on branching, execution path, the `MoveNext()` call that occurs behind the scenes will move from one state to another.
