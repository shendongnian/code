    public string campDiff(string lineStr1, string lineStr2)
    {
        ...
        {
            if (Double.TryParse(lineArr1[i].Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture, out num1))
            {
                if (Double.TryParse(lineArr2[i].Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture, out num1)
                {
                    number = num2 - num1;
                    if (number > 0)
                    {
