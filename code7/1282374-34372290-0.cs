    ...
    public class MatchMaking
    {
        ...
        public IList<Match> GetMatches()
        {
            ...
            var unorderedMatchesArray = unorderedMatches.ToArray();
            ...
            for (var i = 0; i < unorderedMatchesArray.Length; i++)
            {
                if (lastAdded == null)
                {
                    lastAdded = unorderedMatchesArray[i];
                    orderedMatches.Add(unorderedMatchesArray[i]);
                    unorderedMatchesArray[i] = null;
                    continue;
                }
                var remainingMatches = unorderedMatchesArray.Where(m => m != null);
                ...
                var index = unorderedMatchesArray.ToList().IndexOf(mostDifferentFromLastAdded);
                lastAdded = unorderedMatchesArray[index];
                orderedMatches.Add(unorderedMatchesArray[index]);
                unorderedMatchesArray[index] = null;
            }
