	public class DynamicResourceBindingExtension : MarkupExtension {
		...
		public override object ProvideValue(IServiceProvider serviceProvider) {
			...
			wrapperBinding.Bindings.Add(targetObjectBinding);
			wrapperBinding.Bindings.Add(dynamicResourceBinding);
		...
		private object WrapperConvertValue(object[] values, Type targetType, object parameter, CultureInfo culture) {
			var bindingTargetObject          = values[0];
			var dynamicResourceBindingResult = values[1]; // Passes thru the result of the DynamicResourceBinding
			...
			// (Again, we only needed the other value to set up the resources collection
