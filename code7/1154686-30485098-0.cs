    ...
    2. In the handler, dig in to find the `ScrollViewer`. It will only be present when the `ViewingMode` is set to `Scroll`:
            private void Reader_ViewModeChanged()
            {
                ...
                {
                    var border = mReader.Template.FindName("PART_ContentHost", mReader) as Border;
                    if (border != null && VisualTreeHelper.GetChildrenCount(border) > 0)
                    {
                        var documentScrollViewer = VisualTreeHelper.GetChild(border, 0) as FlowDocumentScrollViewer;
