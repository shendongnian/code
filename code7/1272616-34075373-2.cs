            ...
            foreach (EventHandler<BreadWinnerEventArgs> handler in t.GetInvocationList())
            {
                try
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        throw new DelegateException(handler, e); //Throw the exception to capture the stack trace.
                    }
                }
    ...
    sealed class DelegateException : Exception
    {
        ...
        public DelegateException(Delegate handler, Exception innerException) : base("A delegate raised an error when called.", innerException)
        {
            Handler = handler;
        }
    }
