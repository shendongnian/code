    ...
    private void DrawIn(Graphics g, float x, float y, float width, float height)
    {
        ...
        foreach (var item in Items)
        {
            if (item.Children.Count() > 0)
            {
                ...
                treeMap.DrawIn(g, x + item.TMData.Location.X, y + item.TMData.Location.Y, item.TMData.Size.Width, item.TMData.Size.Height);
            }
            else
            {
                ...
                g.DrawString(item.Name, SystemFonts.DefaultFont, Brushes.Black, x + item.TMData.Location.X, y + item.TMData.Location.Y);
            }
            g.DrawRectangle(Pens.Black, x + item.TMData.Location.X, y + item.TMData.Location.Y, item.TMData.Size.Width, item.TMData.Size.Height);
        }
        g.Flush();
    }
