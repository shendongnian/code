    ...
    using (var mappedFile2 = MemoryMappedFile.CreateFromFile(f2Path, FileMode.OpenOrCreate, null, offset + length))
    {
        using (var reader = mappedFile2.CreateViewStream(offset, length, MemoryMappedFileAccess.Read))
        {
            // Read from MMF
    ...
    using (var mappedFile1 = MemoryMappedFile.CreateFromFile(f1Path,FileMode.Open, null, offset + length))
    {
        ...
        using (var writer = mappedFile1.CreateViewAccessor(offset,
