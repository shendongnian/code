	public class AlertDing {
		...
		public AlertDing(double freq, uint tenthseconds) {
			...
			ushort fmt_BitsPerSample = 16;   // bits per sample
			...
			for (uint i = 0; i < numSamples - 1; i++) {
				...
				for (int channel = 0; channel < fmt_Channels; channel++) {
					data_ShortArray[i + channel] = Convert.ToInt16(amp * Math.Sin(i*period));
				}
			}
	
			// Calculate file and data chunk size in bytes
			data_ChunkSize = (uint)(data_ShortArray.Length * (fmt_BitsPerSample / 8));
			...
			foreach (short dataPoint in data_ShortArray) {
