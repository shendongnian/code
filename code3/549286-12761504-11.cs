    public static class SingleSelectionGroup
    {
        ...
        {
            if(e.OldValue != null)
                RemoveSelector(dependencyObject, e);
    
            if(e.NewValue != null)
                AddSelector((Selector)dependencyObject, (string)e.NewValue);
        }
    
        private static void RemoveSelector(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
        {
            var selector = (Selector) dependencyObject;
            selector.SelectionChanged -= SelectorOnSelectionChanged;
    
            _selectors[(string) e.OldValue].Remove(selector);
        }
