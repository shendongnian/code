...
Here is a working example, which demos the following:
 - multiple symmetrical producers and consumers
...
static class Program
{
	static void Main()
	{
		...
		Task.WaitAll(producers.ToArray()); // may also use async equivalent
		Console.WriteLine("Finished waiting on producers");
		buffer.Complete(); // demo being able to complete the collection
		Task.WaitAll(consumers.ToArray()); // may also use async equivalent
