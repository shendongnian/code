    ...
    class ReadOnlyCollectionMethods:IMethods<ReadOnlyCollection<int>>
    {
      ...
      {
        return new ReadOnlyCollection<int>(BlockCopy(items).OrderBy(s => s).ToList());
      }
    }
    static class Program
    {
      ...
      static void Main()
      {
        ...
        Console.WriteLine("       Array       ReadOnlyCollection   Penalties   Method");
        for(var i = 0; i < arrayStats.Length; ++i)
        {
          Console.WriteLine("{0}    {1}    1 vs {2,4:f1}   {3}", arrayStats[i].Item2, rcStats[i].Item2, rcStats[i].Item2.TotalSeconds / arrayStats[i].Item2.TotalSeconds, arrayStats[i].Item1);
        }
      }
