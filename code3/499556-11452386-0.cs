        ...
        public void PayForOrderWithGiftCoupons(PayForOrderWithGiftCouponsCommand command)
        {
            using (IUnitOfWork unitOfWork = UnitOfWorkFactory.Create())
            {
                ...
                foreach(GiftCoupon coupon in coupons)
                    __giftCouponRepository.Save(coupon);
            }
        }
    }
    public class Order : IAggregateRoot
    {
        ...
        public void MakePaymentWithGiftCoupons(List<GiftCoupon> coupons)
        {
            ...
            _payments.Add(new GiftCouponPayment(Guid.NewGuid(), DateTime.Now, coupons));
        }
    }
    ...
    {
        private readonly List<Guid> _couponIds = new List<Guid>();
        private readonly decimal _amount;
        public override decimal  Amount
        {
	        get { return _amount; }
        }
        public GiftCouponPayment(Guid paymentId, DateTime paymentDate, List<GiftCoupon> coupons)
            : base(paymentId, paymentDate)
        {
            foreach(GiftCoupon coupon in coupons)
                _couponIds.Add(coupon.GiftCouponId);
            _amount = coupons.Sum(c => c.Value);
        }
