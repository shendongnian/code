    public partial class LineMover : Form
    {
      ...
      void  LineMover_MouseDown(object sender, MouseEventArgs e)
      {
        ...
        {
          this.Capture = true;
          Moving = new MoveInfo {Line = this.SelectedLine, StartLinePoint = SelectedLine.StartPoint, EndLinePoint = SelectedLine.EndPoint, StartMoveMousePoint = e.Location };
        }
        RefreshLineSelection(e.Location);
      }
      ...
      {
        var selectedLine = CheckLineSelection(Lines, point);
      ...
      static GraphLine CheckLineSelection(List<GraphLine> lines, Point p)
      {
        var size = 10;
        var zbuffer = new Bitmap(size * 2, size * 2);
        for (var i = 0; i < lines.Count; ++i)
        {
          var line = lines[i];
          using (var g = Graphics.FromImage(zbuffer))
          {
            g.Clear(Color.FromArgb(0));
            var color = Color.Green;
            g.DrawLine(new Pen(color, 3), line.StartPoint.X - p.X + size, line.StartPoint.Y - p.Y + size, line.EndPoint.X - p.X + size, line.EndPoint.Y - p.Y + size);
          }
          if (zbuffer.GetPixel(size, size).ToArgb() != 0)
            return line;
        }
        return null;
      }
