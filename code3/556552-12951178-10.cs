        ...
        public MainWindow()
        {
            ...
            //Set up the data binding to point at Character (Source) and Property Health (via the constructor argument for Binding)
            ...
            //Now we bind any changes to CharacterAttributes, HealthDependency to Character.Helth via the characterHealthBinding Binding
            var bindingExpression = BindingOperations.SetBinding(CharacterAttributes, MyCharacterAttributes.HealthDependency, characterHealthBinding);
            //Store the binding so we can look it up if necessary in a List<BindingExpressionBase> in our CharacterAttributes class, and so it "lives" as long as CharacterAttributes does, too
