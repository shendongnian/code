    ...
    foreach (var prop in propMap)
    {
        if (prop.Item1.Compile()(type1) != prop.Item2.Compile()(type2))
        {
            ...
            var assign = Expression.Lambda<Action<Trade, object>>(Expression.Assign(prop.Item1.Body, Expression.Convert(valueParameterExpression, prop.Item1.Body.Type)), prop.Item1.Parameters.Single(), valueParameterExpression);
