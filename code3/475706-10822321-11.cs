    ...
    namespace CodeProjectHelpProject
    {
        public class Deck
        {
            public struct Card
            {
                ...
                {
                    return (Value <= 0 ? "Joker" : Value == 1 ? "Ace" : Value == 11 ? "Jack" : Value == 12 ? "Queen" : Value == 13 ? "King" : Value.ToString()) + " Of " + Suit + " (" + Rank + ')' ;
                }
                ...
                public static bool operator >(Card a, Card b) { return a.Value > b.Value && a.Rank < b.Rank; }
                public static bool operator <(Card a, Card b) { return a.Value < b.Value && a.Rank > b.Rank; }
    
                public static bool operator >(Card a, int b) { Card c = b; return a.Value > c.Value && a.Rank < c.Rank; }
                public static bool operator <(Card a, int b) { Card c = b; return a.Value < c.Value && a.Rank > c.Rank; }
