    public partial class _Default : System.Web.UI.Page
    {
        ...
        {
            base.OnInit(e);
            rptrTest.ItemDataBound += new RepeaterItemEventHandler(rptrTest_ItemDataBound);
        }
        private DropDownList file1DropDown;
        void rptrTest_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            // Find the DropDownList in the repeater's ItemTemplate so we can manipulate it.
            ...
            // Get all of the strings starting with the current ID.
            IEnumerable<string> itemSpecificStrings =
                from string s
                in dataItem.AListOfStrings
                where s.StartsWith(dataItem.ID.ToString())
                select s;
            // Add the strings to whichever DropDownList we need to modify.
            currentDropDownList.Items.AddRange((from s in itemSpecificStrings select new ListItem(s)).ToArray());
        }
