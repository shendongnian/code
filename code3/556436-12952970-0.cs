    ...
    internal class BindingEngine<T>
    {
        ...
        public BindingEngine(T destination, object source, int srcId)
        {
            _source = source;
            ...
            foreach (PropertyDescriptor property in destinationProperties)
            {
                ...
                {
                    var srcProp = _sourceProperties[attribute.PropertyName];
                    _srcToDestMapping[attribute.PropertyName] = property;
                    //Also listen for INotifyPropertyChanged event on the source property
                    srcProp.AddValueChanged(source, SourcePropertyChanged);
                }
            }
        }
        void SourcePropertyChanged(object sender, EventArgs e)
        {
