    public static string Format(IFormatProvider provider, string format, params object[] args)
    {
        ...
        foreach (Token token in tokens)
        {
            switch (token.TokenType)
            {
                ...
                //  token.Text is the format string inside ('' in the first example, 'format' in the second example)
                case TokenType.Index:
                    {
                        object arg = args[token.Index];
                        IFormattable formattable = arg as IFormattable;
                        if (formattable != null && token.Text.Length > 0)
                        {
                            // If the argument is IFormattable we pass it the format string specified with the index
                            sb.Append(formattable.ToString(token.Text, provider));
                        }
                        else
                        {
