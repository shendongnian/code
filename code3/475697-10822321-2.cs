    ...
    namespace CodeProjectHelpProject
    {
        public class Deck
        {
            public struct Card
            {
                ...
                public static readonly Card.Suit[] Suits = ( Card.Suit[])Enum.GetValues(typeof(Card.Suit));
                ...
                public static readonly Card[] Jokers = new Card[] { new Card(CharJoker, Suit.Spades), new Card(CharJoker, Suit.Hearts) };
    
                public char Value { get; internal set; }
    
                public Card.Suit Suiet { get; internal set; }
    
                public int Rank { get { return Convert.ToInt32(Value); } }
    
                public Card(char value, Suit suite) : this()
                {
                    if (!Enum.IsDefined(typeof(Suit), suite)) throw new Exception("Invalid Suite");
                ...
                public Card(int value, Suit suite) : this(unchecked((char)value), suite) { }
    
                public Card(byte value, Suit suite) : this((char)value, suite) { }
            ...
            {
                Cards = new List<Card>();
                foreach (Card.Suit s in Card.Suits)
        ...
        public static class Extensions
        {
            ...
            public static void Shuffle(this List<CodeProjectHelpProject.Deck.Card> Cards, int times = 0)
            {
                do
                {
                    foreach (CodeProjectHelpProject.Deck.Card.Suit s in CodeProjectHelpProject.Deck.Card.Suits)
                    {
