    void Main()
    {
        ...
        using (var tfs = TfsTeamProjectCollectionFactory.GetTeamProjectCollection(new Uri(CollectionAddress)))
        {
            ...
            //You should be able to re-factor this with "Iteration" for getting those too.
            var nodesXml = server.GetNodesXml(
                nodes.Where(node => node.Name == "Area").Select(node => node.Uri).ToArray(),
                true);
    
            var areaPathAndId =
                XElement.Parse(nodesXml.OuterXml)
                .Descendants("Node")
                .Select(xe => new { Path = xe.Attribute("Path").Value, ID = xe.Attribute("NodeID").Value, })
                .Dump();        
        }
    }
