    ...
    class ExpressionsTest
    {
        ...
        internal static void Main()
        {
            ...
            sortedEvents = _allEvents.SortEvents(SortedBy.Descending, 
                ...
                evt => evt.City,
                evt => HelperFunctions.strToTZDateTime(evt.EventStartStr, evt.EventTimeZone).ToIsoFormat());  //I call ToIsoFormat() to sort the dates, which is not ideal, but is efficient nonetheless.  
            Print(sortedEvents);
            Console.Read();
        }
    ...
    internal static class BoringEventExtensions
    {
        ...
        public static IEnumerable<BoringEvent> SortEventsAsc<T>(this IEnumerable<BoringEvent> events, 
            params Expression<Func<BoringEvent, T>>[] expressions)
        {
            IOrderedEnumerable<BoringEvent> sorted = null;
            for(int i = 0; i < expressions.Count(); i++)
            {
                Expression<Func<BoringEvent, T>> exp = (Expression<Func<BoringEvent, T>>)expressions[i];
        ...
        public static IEnumerable<BoringEvent> SortEventsDesc<T>(this IEnumerable<BoringEvent> events,
            params Expression<Func<BoringEvent, T>>[] expressions)
        {
            IOrderedEnumerable<BoringEvent> sorted = null;
            for (int i = 0; i < expressions.Count(); i++)
