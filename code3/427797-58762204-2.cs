    static DateTime GetNextRun(int hour, int min, bool isDaily, bool isWeekly, bool isMonthly, bool isLastDayOfMonth, int collectionDay)
            {
                if (collectionDay < 2)
                {
                    throw new Exception("The collection Days is invalid.");
                }
                ...
                else if (isMonthly)
                {
                    ...
                    while (true)
                    {
                        switch (GetDaysInMonth(nextDate))
                        {
                            case 28:
                                if (collectionDay > 536870910)
                                {
                                    throw new Exception("The collection Days is invalid.");
                                }
                                break;
