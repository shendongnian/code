    public void Populate(Guid UserId)
		{
			...
			{
				con.Open();
				using (SqlCommand RowCount = new SqlCommand("SELECT COUNT([UserId]) FROM aspnet_Membership WHERE [UserId]=@UserId", con))
				{
					SqlParameter param = RowCount.Parameters.Add(new SqlParameter("@UserId", SqlDbType.UniqueIdentifier));
					param.Value = UserId;
							...
							using (SqlDataReader rdr = sqlCommand.ExecuteReader())
							{
								if (rdr.HasRows)
								{
									while (rdr.Read())
									{
										...
										this.Address = common.Coalesce(rdr["Address"], "");
									}
								}
							}
