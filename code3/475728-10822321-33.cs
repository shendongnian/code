        ...
        namespace CodeProjectHelpProject
    {
        ...
        {
            //Exceptions
            internal class DeckException : Exception { public DeckException(string message) : base(message) { } }
            ...
            public struct Card
            {
                ...
                public Card(char value, CardSuit suit)
                    : this()
                {
                    if (!Deck.SuitOrder.Contains(suit)) throw new CardException("Invalid Suite");
            ...
            public static readonly Deck.CardSuit[] SuitOrder = new CardSuit[] { CardSuit.Spades, CardSuit.Clubs, CardSuit.Hearts, CardSuit.Diamonds };
            ...
            public Deck(bool usesJokers = false, bool shuffle = false)
            {
                ...
                else foreach (Deck.CardSuit s in Deck.SuitOrder) foreach (char c in Enumerable.Range(1, 13)) Cards.Add(new Card(c, s));
        ...
        public static class Extensions
        {
            ...
            public static void Shuffle(this List<CodeProjectHelpProject.Deck.Card> Cards, int times = 0)
            {
                do
                {
                    foreach (CodeProjectHelpProject.Deck.CardSuit s in CodeProjectHelpProject.Deck.SuitOrder)
                    {
