    ...
    public partial class Db<S, T> : IDb where S: IDbConnection, new () 
                                        where T: IDbCommand, new ()
    {
        ...
        public R? ExecuteAndGet<R>(string query, Dictionary<string, object> parameters, 
                                   out string possibleError) where R : struct
        {
            possibleError = null;
            try
                ...
                using (var conn = new S())
                {
                    ...
                    using (var cmd = new T())
                    {
                        ...
                        if (table.Rows.Count == 0)
                            return default(R); //or use your version of default value in case nothing is read
