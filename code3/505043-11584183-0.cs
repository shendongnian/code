        ...
        string proxy = "59.36.183.178";//host;
        int proxyPort = 8081;//443;
        string proxy2 = "222.74.212.66";//host;
        int proxyPort2 = 808;//443;
        ...
        sslStream.AuthenticateAsClient(proxy2);
        // Establish proxy-target tunnel
        tunnelRequest = Encoding.UTF8.GetBytes(String.Format("CONNECT {0}:443  HTTP/1.1\r\nHost: {0}\r\n\r\n", host));
        sslStream.Write(tunnelRequest, 0, tunnelRequest.Length);
        sslStream.Flush();
        // Read response to CONNECT request
        // There should be loop that reads multiple packets
        ...
        SslStream sslStream2 = new SslStream(sslStream);
