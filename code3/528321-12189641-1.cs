    ...
    public class ExcelProvider : IEnumerable<ExcelRow>
    {
        ...
        private void Load()
        {
            ...
            rows.Clear();
            Program.logger.Debug("Inside the ExcelProvider class Load method");
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                Program.logger.Debug("In load method OleDbConnection block");
                try
                {
                    Program.logger.Debug("In Load method try block");
                    conn.Open();
                }
                catch (Exception ex)
                {
                ...
                {
                    Program.logger.Debug("In Load method OleDbCommand block");
                    cmd.CommandText = "select * from [" + sheet + "$]";
                    using (OleDbDataReader reader = cmd.ExecuteReader())
                    {
                        Program.logger.Debug("In load method OleDbDataReader block");
                        while (reader.Read())
                        {
