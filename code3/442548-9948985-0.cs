    public static IEnumerable<IEnumerable<T>> Rotate (this List<T> source)
    {
      ...
      //similiar to list.Skip(i-1), but using list's indexer access to reduce iterations
    public static IEnumerable<T> TakeFrom(this List<T> source, int index)
    {
      ...
      //similiar to list.Take(i), but using list's indexer access to reduce iterations    
