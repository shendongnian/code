    ...
    if (tablecollection != null)
    {
        foreach (HtmlNode table in tablecollection)
        {
            ...
            {
                // combine the child 'td' elements into an array, join with the pipe to 
                // create the output in 'val|val|val' format.
                store = string.Join("|", row.DescendantNodes().Where(
                    desc => desc.Name.Equals("td", StringComparison.OrdinalIgnoreCase)).Select(
                        desc => desc.InnerText));
                // you can probably get rid of the 'store' variable as it's no longer
                // necessary to store the value of the tables cells over the iteration.
                sw.Write(store);
                sw.WriteLine();
            }
        }
