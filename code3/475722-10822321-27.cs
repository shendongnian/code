    ...
    namespace CodeProjectHelpProject
    {
        public class Deck
        {
            ...
            public struct Card
            {
                ...
                {
                    return "Card, " + (Value <= 0 ? "Joker" : Value == AceValue ? "Ace" : Value == JackValue ? "Jack" : Value == QueenValue ? "Queen" : Value == KingValue ? "King" : Value.ToString()) + " Of " + Suit + " (" + Rank + ')' ;
                }
            ...
            public static bool HasAllCards(Deck d) { foreach (Deck.CardSuit s in Deck.Suits) foreach (int c in Enumerable.Range(d.HasJokers ? 0 : 1, 13)) if (!d.Cards.Contains(new Card(c, s))) return false; return true; }
            ...
            public static List<Card> operator - (Deck d, int amount) { return d.Take(amount, true); }
            public static List<Card> operator +(Deck d, IEnumerable<Card> cards) { d.Cards.AddRange(cards); return d.Cards; }
    
            public static List<Card> operator -(Deck a, Deck b) { a.Cards.ForEach(c => b.Cards.Remove(c)); return a.Cards; }
            public static List<Card> operator +(Deck a, Deck b) { a.Cards.AddRange(b.Cards); return a.Cards; }
