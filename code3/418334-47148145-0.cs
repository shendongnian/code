    public static class MultidimensionalArrayExtensions
    {
        ...
        public static void ForEach<T>(this Array array, Action<T, int[]> action) => ForEach(array, action, new int[] { }, null);
        private static void ForEach<T>(Array array, Action<T, int[]> action, int[] externalCoordinates, Array masterArray)
        {
            ...
            else
            {
                for (int i = 0; i < dimensionSizes[0]; i++)
                {
                    var subArray = Array.CreateInstance(typeof(T), dimensionSizes.Skip(1).ToArray());
                    ForEach(subArray, action, externalCoordinates.Concat(new[] { i }).ToArray(), masterArray ?? array);
                }
            }
        }
