    ...
    public sealed class SingleInstanceHandler
    {
        ...
        public void Connect (string[] args)
        {
            ...
            using (ipcMutex = new Mutex(true, ipcMutexGuid, out success))
            {
                // if the global mutex was already created, send args to the existing instance
                if (!success)
        ...
        private void sendArgsToExistingInstance (string[] args)
        {
            ...
            try
            {
                ...
                if (!pipeClient.IsConnected)
                    throw new Exception("time out did not throw exception");
