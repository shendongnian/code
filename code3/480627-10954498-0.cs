    var doubleAR = new List<List<Double>>()
    {
       ...
       var asXml = new XElement("Data",
                                doubleAR.Select (dList => dList.Select (dbl => dbl.ToString()) )
                                        .Select(sList => new XElement("Doubles", sList.Select (sl => new XElement("Double", sl) )))
                               );
       ...
       List<List<Double>> asDoubleArray =
             XDocument.Parse( asXml.ToString() )
                      ...
                      .Select (eDL => eDL.Descendants("Double")
                                         .Select (dl => double.Parse(dl.Value))
                                          .ToList())
                      .ToList();
