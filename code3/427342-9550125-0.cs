	static Dictionary<string, List<int>> GetDuplicates(string value)
	{
		...
		for(int i = 0; i < value.Length; i++)
		{
			for (int slength = 2; slength < (value.Length - i) / 2 + 2; slength++)
			{
				...
				{
					int nextOccurrence = value.IndexOf(littleString, i + slength);
					...
					if (nextOccurrence != -1)
					{
						...
						while ((nextOccurrence = value.IndexOf(littleString, nextOccurrence + slength)) != -1)
						{
