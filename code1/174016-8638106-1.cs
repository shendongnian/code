    public class XmlColor
    {
        ...
        public static Color ToColor(string value)
        {
            try
            {
                if (value[0] == '#')
                {
                    if (value.Length == 7) // preserve named color value if possible
                        return ColorTranslator.FromHtml(value);
                    else if (value.Length == 9)
                        return Color.FromArgb(Int32.Parse(value.Substring(1), System.Globalization.NumberStyles.HexNumber));
                    return Color.FromArgb(Int32.Parse(value.Substring(1), System.Globalization.NumberStyles.HexNumber));
                }
                else
                {
