...
.
    internal class Sorter<E>
    {
        ...
        public IOrderedQueryable<E> Sort(IQueryable<E> list, params Tuple<string, SorterSortDirection>[] names) 
        { 
            IOrderedQueryable<E> result = null; 
            foreach (var entry in names)
            {
                result = result == null ? SortFirst(entry.Item1, entry.Item2, list) : SortNext(entry.Item1, entry.Item2, result); 
            } 
            return result; 
        } 
        private IOrderedQueryable<E> SortFirst(string name, SorterSortDirection direction, IQueryable<E> source) 
        { 
            return (direction == SorterSortDirection.Descending) ? firstPassesDesc[name].Invoke(source) : firstPasses[name].Invoke(source);
        } 
        private IOrderedQueryable<E> SortNext(string name, SorterSortDirection direction, IOrderedQueryable<E> source) 
        {
            return (direction == SorterSortDirection.Descending) ? nextPassesDesc[name].Invoke(source) : nextPasses[name].Invoke(source); 
        } 
