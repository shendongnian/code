    ...
    static extern bool ReadFile(IntPtr handle, IntPtr buffer, uint numBytesToRead, ref uint numBytesRead, IntPtr overlapped);
    [DllImport("kernel32.dll", SetLastError=true)]
    [return:MarshalAs(UnmanagedType.Bool)]
    static extern bool SetFilePointerEx(IntPtr hFile, long liDistanceToMove, ref long lpNewFilePointer, uint dwMoveMethod);
    unsafe bool read(FileStream fs, ushort[] buffer, int offset, int count)
    {
      ...
      try {
        long unused;
        if (!SetFilePositionEx(nativeHandle, offset, ref unused, 0);
        fixed (ushort* pFirst = &buffer[offset])
          if (!ReadFile(nativeHandle, new IntPtr(pFirst), bytesToRead, ref bytesToRead, IntPtr.Zero)
            return false;
