    public static string GenerateSqlStatement(this SqlCommand cmd)
    {
    	return $@"EXECUTE sp_executesql N'{cmd.CommandText.Replace("'", "''")}'{cmd.Parameters.GenerateSqlParameters()}";
    }
    
    private static string GenerateSqlParameters(this SqlParameterCollection col)
    {
    	if (col.Count == 0)
    		return string.Empty;
    	...
    	{
    		parameters.Add($"{param.ParameterName}{GenerateSqlParameterPrecisionAndScale(param)}");
    		parameterValues.Add($"{param.ParameterName} = {GenerateSqlParameterValue(param)}");
    	}
    	return $",N\'{string.Join(",", parameters)}\',{string.Join(",", parameterValues)}";
    }
    
    private static object GenerateSqlParameterPrecisionAndScale(SqlParameter param)
    {
    ...
    private static string GenerateSqlParameterValue(SqlParameter param)
    {
    	switch (param.SqlDbType)
    	{
    		case SqlDbType.Char:
