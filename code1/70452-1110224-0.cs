    ...
    [DllImport("coredll", EntryPoint = "FormatMessage")]
    ...
    private static Win32Exception CreateWin32Exception(int error)
    {
        ...
        try
        {
            if (NoSetLastErrorFormatMessage(FormatMessageFlags.IgnoreInserts | FormatMessageFlags.FromSystem | FormatMessageFlags.AllocateBuffer, IntPtr.Zero, (uint)error, 0, out buffer, 0, IntPtr.Zero) == 0)
            {
