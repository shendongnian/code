    	...
    	public static class Enumerable
    	{
    		...
    		public static Converter<TInput, TOutput> GetConverterDelegate<TInput, TOutput>()
    		{
    			...
    			MethodInfo[][] conversion_op_sets = new MethodInfo[2][] {
    				type_pair.target_type.GetMethods( BindingFlags.Static | BindingFlags.Public ),
    				type_pair.source_type.GetMethods( BindingFlags.Static | BindingFlags.Public )
    			};
    			...
    			for (int s = 0; s < conversion_op_sets.Length; s++) {
    				...
    				for (int m = 0; m < conversion_ops.Length; m++)
    				{
    					...
    					if ((mi.Name == "op_Explicit" || mi.Name == "op_Implicit") && 
    						...
    						mi.GetParameters()[0].ParameterType == type_pair.source_type) //Assuming op_Explicit and op_Implicit always have exactly one parameter.
    					{
