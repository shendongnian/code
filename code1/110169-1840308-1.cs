    ...
    for (int i = 0; i < strCompositeKeyField.Length; i++)
    {
        ...
        strSelect.AppendFormat("{0} =", _strHeaderLineSplitedArray[(int)arrayListCompositeKeyIndex[i]]);
        ...
        {
        strSelect.AppendFormat("'{0}'", strReadDataLineSplited[(int)arrayListCompositeKeyIndex[i]]);
        boolKeyProcess = true;
        }
