    ...
    public partial class frmTestAutocomplete : Form
    {
        ...
        private void testCombo_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            //3).binding data source erases text, so now we need to put the user's text back,
            oBox.Text = sBoxText;
            oBox.SelectionStart = sBoxText.Length; //4). need to put the user's cursor back where it was.
    
            //show the user the new filtered list.
            oBox.DroppedDown = true; //5). show all the new options!
