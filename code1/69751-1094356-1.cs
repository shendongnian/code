...
<strike>Note that - as written - it won't work across assembly boundaries.  It can be done using essentially the same code, however - you just need to supply the assembly-qualified name to the Type.GetType() method.</strike> (fixed - use AssemblyQualifiedName instead of FullName; assumes that interface and implementing class are in same assembly.)
