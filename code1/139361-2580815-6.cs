    ...
    namespace CAM_Products.General_Controls
    {
        public class DataGridViewWithDraggableRows : DataGridView
        {
            ...
            {
                get { return (int)_selectionPen.Width; }
                set { _selectionPen.Width = value; }
            }
            #endregion
            ...
            private void dataGridView1_Paint_Selection(object sender, PaintEventArgs e)
            {
                if (_selectedRow != null && _selectedRow.DataGridView == this)
                {
                    Rectangle displayRect = GetRowDisplayRectangle(_selectedRow.Index, true);
                    if (displayRect.Height == 0)
                        return;
    
                    int heightAdjust = (int)Math.Ceiling((float)SelectionWidth / 2);
                    e.Graphics.DrawRectangle(_selectionPen, displayRect.X, displayRect.Y - heightAdjust,
            ...
            private void dataGridView1_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    DoDragDrop(dragObject, DragDropEffects.Move); //TODO: Any way to make this *not* happen if they only click?
                }
            }
            ...
            private void dataGridView1_Paint_RowDivider(object sender, PaintEventArgs e)
            {
                if (_predictedInsertIndex != null)
                {
                    ...
                    e.Graphics.FillRectangle(_dividerBrush, locationX, locationY, width, DividerHeight);
                }
            }
