    public partial class Form2 : Form
    {
        public Form2()
        {
            ...
            IEnumerable<CompanyWithEmployees> whereAreMichaels = companiesWithEmployees.Where(cx => cx.Employees.Any(px => px.PersonName == "Michael"));
            string michaelsCompanies = string.Join(", ", whereAreMichaels.Select(cx => cx.CompanyInfo.CompanyName).ToArray());
            MessageBox.Show("Company(s) with employee Michael : " + michaelsCompanies);
            Person findAga = companiesWithEmployees.Find(company => company.CompanyInfo.CompanyName == "Muhlach").Employees.Find(px => px.PersonName == "Aga");
            if (findAga != null)
