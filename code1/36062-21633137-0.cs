    public class QueryProvider : IQueryProvider
    {
        ...
        private static object[] GetNewParameterList(ref string query, List<DbParameter> parameters)
        {
            ...
            foreach (var dbParameter in parameters.Where(p => p.DbType == System.Data.DbType.Int32 && !p.IsNullable))
            {
                query = query.Replace(dbParameter.ParameterName, dbParameter.Value.ToString());
            ...
            for (var i = 0; i < newParameters.Count; i++)
            {
                ...
                var newName = "@p" + i;
                query = query.Replace(parameter.ParameterName, newName);
            }
