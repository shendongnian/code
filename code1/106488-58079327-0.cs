    public static class FileSystemWatcherExtensions
    {
        ...
        {
            var cancelattions = new Dictionary<string, CancellationTokenSource>(
            ...
            async void FileSystemWatcher_Event(object sender, FileSystemEventArgs e)
            {
                ...
                lock (locker)
                {
                    if (cancelattions.TryGetValue(key, out var existing))
                    {
                        existing.Cancel();
                    }
                    cancelattions[key] = cts;
                }
                try
                {
