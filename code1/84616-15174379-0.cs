            public static string CreateTABLEPablo(string connectionString, string tableName, System.Data.DataTable table)
        {
            ...
            using (System.Data.OleDb.OleDbConnection connection = new System.Data.OleDb.OleDbConnection(connectionString))
            {
                ...
                //"CONSTRAINT FKIdTramo FOREIGN KEY (IdTramo) REFERENCES Tramos ON UPDATE CASCADE ON DELETE CASCADE, CONSTRAINT FKIdProyecto FOREIGN KEY (IdProyecto) REFERENCES Proyectos ON UPDATE CASCADE ON DELETE CASCADE)"
                for (int i = 0; i < table.ParentRelations.Count; i++)
                {
                    System.Console.WriteLine(table.ParentRelations[i]);
                }
                sqlsc += pks;
                connection.Close();
