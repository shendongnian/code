    ...
    public static void StopServiceAndWaitForExit(string serviceName)
    {
      using (ServiceController controller = new ServiceController(serviceName))
      {
        ...
        {
          // EDIT:
          // There is no need for waiting in a separate thread, because MSDN says
          // "The handles are valid until closed, even after the process or thread
          // they represent has been terminated."
          // (http://msdn.microsoft.com/en-us/library/windows/desktop/ms684868%28v=vs.85%29.aspx),
          // so to keep things in the same thread, the process HANDLE should be
          // opened from the process id before the service is stopped, and the Wait
