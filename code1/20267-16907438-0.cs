    ...
    namespace ConversionExtensions
    {
        public static class StringExtensions
        {
            public static byte[] ToBytes(this string hexString)
            {
                ...
                {
                    throw new ArgumentException("Hexadecimal string must contain an even number of digits to be valid.");
                }
                ...
                using (MemoryStream data = new MemoryStream(hexString.Length / 2))
                {
                    for (int index = 0; index < hexString.Length; index += 2)
                    {
                        ...
                        {
                            throw new ArgumentException("An invalid digit was encountered. Valid hexadecimal digits are 0-9 and A-F.");
                        }
                        else
                        {
