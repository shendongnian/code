	...
	public void Multi_Save_Test()
	{
		...
		using (var holdingstream = new MemoryStream())
		{
			...
			//open the holding stream, make a change, and resave it
			using (var pack = new ExcelPackage(holdingstream))
			{
			...
			//open the holding stream, make a change, and do a FINAL save
			using (var pack = new ExcelPackage(holdingstream))
			{
				var ExSheet = pack.Workbook.Worksheets["Data"];
