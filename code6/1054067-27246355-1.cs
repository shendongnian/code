        ...
        public static Bitmap CreateBitmap(byte[] data, int width, int height, int stride, PixelFormat format32BppArgb)
        {
            var arrayHandle = GCHandle.Alloc(data, GCHandleType.Pinned);
            var bmp = new Bitmap(width, height, stride, format32BppArgb, arrayHandle.AddrOfPinnedObject());
            arrayHandle.Free();
            return bmp;
        }
    }
