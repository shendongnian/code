	...
	namespace TestForm
	{
		public partial class testForm : Form
		{
			...
			private void ItemCheck(object sender, ItemCheckEventArgs event)
			{
				if (sender == m_checkedListBox)
				{
					if (!m_checkedListBox.Sorted)
					{
						...
						m_underList[event.Index].IsChecked = (event.NewValue != CheckState.Unchecked);
						//	Monitoring
						Debug.WriteLine(m_underList[event.Index].ShowVarState());
					}
					else
					{
						//	If sorted... DIY
