    class Program
    {
        static void Main(string[] args)
        {
            ...
            var result = rows
                ...
                .Select(item => new { RowId = item.Id, OldData = item.Count == 1 ? null : item.FirstRow.Data, NewData = item.LastRow.Data, RowMod = item.LastRow.RowMod });
            // Test
            Console.WriteLine("RowID\tOldData\tNewData\tRowMod");
            foreach (var item in result)
            {
                Console.WriteLine("{0}\t'{1}'\t'{2}'\t'{3}'",item.RowId,item.OldData,item.NewData,item.RowMod);
            }
        }
    }
