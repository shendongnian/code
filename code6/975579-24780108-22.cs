    ...
    {
        var pointX = (float)(circlePoint.X + Math.Cos(startAngle + sign * (double)i / degreeFactor) * radius);
        var pointY = (float)(circlePoint.Y + Math.Sin(startAngle + sign * (double)i / degreeFactor) * radius);
        points[i] = new PointF(pointX, pointY);
        graphics.DrawRectangle(pen, pointX, pointY, 1, 1);
    }
