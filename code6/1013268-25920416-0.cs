        static IEnumerable<Image> GetThumbnails(Stream stream)
        {
            ...
            {
                if (prevOffset > -1)
                    yield return GetImageAt(
                        allImages, prevOffset, offset - prevOffset);
                prevOffset = offset;
            }
            if (prevOffset > -1)
                yield return GetImageAt(
                    allImages, prevOffset, allImages.Length - prevOffset);
        }
        static Image GetImageAt(byte[] data, int start, int end)
        {
            using (var ms = new MemoryStream())
            {
                ms.Write(data, start, end);
                return Image.FromStream(ms);
            }
        }
        static IEnumerable<int> GetBytePatternPositions(
