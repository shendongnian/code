    ...
    {
        if (Ks == null) return;
        pb_image.Image = new Bitmap(pb_image.ClientSize.Width, pb_image.ClientSize.Height);
        ...
        for (int y = 0; y < s1.Height; y++)
        {
            for (int x = 0; x < s1.Width; x++)
            {
                ...
                Color c = colors[(Ks[x, y] + colors.Count + koffset) % (colors.Count)];
