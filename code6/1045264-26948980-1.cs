        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int row = 0; row < data.Length; row++)
            {
                ...
                e.Graphics.DrawString(data[row].Name, textFont, textBrush, curRect, format);
                //Print Price (DrawRect around first one)
                curRect.Offset(colPadding + colWidths[0], 0F);
                curRect.Width = colWidths[1];
                e.Graphics.DrawString(string.Format("{0:C}",data[row].Price), textFont, textBrush, curRect, format);
                ...
                e.Graphics.DrawString(string.Format("{0:MMMM dd, yyyy}",data[row].Date), textFont, textBrush, curRect, format);
