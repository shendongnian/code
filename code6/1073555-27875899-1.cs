    ...
    private void panel1_MouseDown(object sender, MouseEventArgs e)
    {
        if (e.Button == System.Windows.Forms.MouseButtons.Left)
        {
            ...
            MatrixOrder MO = MatrixOrder.Prepend;
            MatrixOrder MA = MatrixOrder.Append;
            // graphics transfomation
            viewMatrix.Multiply(translateOrigin, MO);
            viewMatrix.Multiply(scaleMatrix, MO); 
            viewMatrix.Multiply(translateBack, MO); 
            // store the next point reversal:
            viewMatrixRev.Multiply(translateBack, MA); 
            viewMatrixRev.Multiply(scaleMatrixRev, MA); 
            viewMatrixRev.Multiply(translateOrigin, MA); 
        }
        else
        {
            // reset
