					...
					try
					{
						...
						using (HttpWebResponse resp = (HttpWebResponse)req.GetResponse())
						{
							if (req.HaveResponse)
							{
								...
								// Again get the response from the server
								using (HttpWebResponse resp2 = (HttpWebResponse)req.GetResponse())
								{
									using (StreamReader stw = new StreamReader(resp2.GetResponseStream()))
									{
					...
					if (sPostData.Count() % 2 == 0)
					{
						...
						using (HttpWebResponse resp = (HttpWebResponse)req.GetResponse())
						{
							if (req.CookieContainer.Count > nInitialCookieCount)
							{
								...
								string sSourcePage = "";
								// Get the response from the server again
								using (HttpWebResponse resp2 = (HttpWebResponse)req.GetResponse())
								{
									using (StreamReader stw = new StreamReader(resp2.GetResponseStream()))
									{
