    ...
    private void HueRotateButton_Click(object sender, EventArgs e)
    {
        ...
        // Get the original image from the picture box control, ...
        ...
        for (var y = 0; y < height; y++)
        {
            ...
            for (var x = 0; x < width; x++, offset += 4)
            {
                ...
                double b = Marshal.ReadByte(scan0, offset);
                double g = Marshal.ReadByte(scan0, offset + 1);
                double r = Marshal.ReadByte(scan0, offset + 2);
                ...
                Marshal.WriteByte(scan0, offset, (byte)br);
                Marshal.WriteByte(scan0, offset + 1, (byte)gr);
                Marshal.WriteByte(scan0, offset + 2, (byte)rr);
            }
        }
        
        // Bitmap editing is finished, "lock" the image again
