    public static int GetSize(Type type, string arrayName, bool instance)
    {
        ...
        // We don't know which constructor does the initialization, so
        // we check each one
        ConstructorInfo[] constructors = typeof(MyClass).GetConstructors(bindingFlags);
        foreach (ConstructorInfo constructor in constructors)
        {
            ...
            {
                // If there is a call to another constructor, then this isn't the
                // method we are looking for :)
                if (instructions[i].OpCode == OpCodes.Call && (instructions[i].Operand as ConstructorInfo) != null)
                {
            ...
            // Access to the array wasn't found. Let's look
            // at the next constructor
            ...
            // Calc the size. There are various opcodes that
            // could be used
            int size;
            ...
            {
                // The size of the array was calculated in some other way.
                // Not supported
                throw new Exception();
            }
            return size;
