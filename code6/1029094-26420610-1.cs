    ...
    string query = "update details set  result=@finalresult where student_id = 11";
    using(SqlConnection con = new SqlConnection(connectionstring))
    {
       SqlCommand cmd = new SqlCommand(query,con);
       con.Open();
       cmd.Parameters.Add(new SqlParameter("@result", initial_result.ToString() + finalresult.ToString()));
