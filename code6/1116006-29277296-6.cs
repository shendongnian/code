    ...
    public class VISCIIEncoder : Encoder
    {
        ...
        public override int GetByteCount(char[] chars, int index, int count, bool flush)
        {
            ...
            for (; index < count2; index++)
            {
                ...
                if (buffer != null)
                {
                    // If we have a High/Low surrogates couple, we pass them together
            ...
            for (; charIndex < charCount2; charIndex++)
            {
                ...
                if (buffer != null)
                {
                    ...
                    if (char.IsLowSurrogate(ch))
                    {
                        ...
                        {
                            HandleFallback(bytes, ref byteIndex2);
                        }
                        buffer = null;
                        continue;
                    }
        ...
        private void HandleFallback(byte[] bytes, ref int byteIndex)
        {
            while (FallbackBuffer.Remaining > 0)
            {
