	public static Task<List<Category>> GetAllCategoriesAsync(Uri CoursesLink)
	{
    ...
    //...
	var task = GetAllCategoriesAsync(new Uri(""));
	task.Wait();
	var categories = task.Result;
    // OR:
	var categories = GetAllCategoriesAsync(new Uri("")).Result;
