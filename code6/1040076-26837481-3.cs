    private DataTable TransposeNumericColumnIntoColumns(this DataTable dt, int indexColumnToEstablishDuplicateRows, int numericColumnIdToTranspose, string transposedColumnName)
    {
        ...
        {
            int duplicateRow = GetRowIndexById(result, indexColumnToEstablishDuplicateRows, Convert.ToInt32(result.Rows[i][indexColumnToEstablishDuplicateRows]),i + 1);
            if (duplicateRow > -1)
            {
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            int rowIndex = GetRowIndexById(result, indexColumnToEstablishDuplicateRows, Convert.ToInt32(dr[indexColumnToEstablishDuplicateRows]));
        ...
        //result.Columns.Remove(transposedColumnName);
    ...
    private void button1_Click(object sender, EventArgs e)
    {
        ...
        dtOriginal.Columns.Add(transposedColumnName,typeof(int));
    
        DataRow dr = dtOriginal.NewRow();
        dr["emp_num"] = 22;
        dr["name"] = "John";
        dr["status"] = 0;
        dr[transposedColumnName] = 4;
        ...
        dr[transposedColumnName] = 4;
        dtOriginal.Rows.Add(dr);
        DataTable result = indexColumnToEstablishDuplicateRows(dtOriginal, 0, 3, "session_id");
