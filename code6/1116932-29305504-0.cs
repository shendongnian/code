            if (!String.IsNullOrWhiteSpace(model.DrillDown))
            {
                switch (model.SelectedInterval)
                {
                    case TimeInterval.Weekly:
                        ...
                        model.ThroughDate = model.FromDate.AddDays(7);
                        break;
            ...
            {
                drilldownQuery = drilldownQuery.Where(session => session.Drivers.Any(driver =>
                    driver.FirstName.StartsWith(model.FirstName) &&
                    driver.LastName.StartsWith(model.LastName))
                    );
            }
