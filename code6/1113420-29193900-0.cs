    public class CustomCollection<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        protected readonly EqualityComparer<TKey> Comparer = EqualityComparer<TKey>.Default;
        ...
        public void Add(TKey key, TValue value)
        {
            if (IsUsingList)
            {
                ...
                if (List.Any(x => Comparer.Equals(x.Key, key)))
                {
        ...
        public bool Remove(TKey key)
        {
            if (IsUsingList)
            {
                ...
                {
                    if (Comparer.Equals(List[ix].Key, key))
                    {
                        List.RemoveAt(ix);
                        return true;
                    }
        ...
        public void ICollection<KeyValuePair<TKey, TValue>>.Add(KeyValuePair<TKey, TValue> item)
        {
        ...
        public bool ICollection<KeyValuePair<TKey, TValue>>.Contains(KeyValuePair<TKey, TValue> item)
        {
            if (IsUsingList)
            {
                if (item.Key == null)
                ...
                return List.Any(x => Comparer.Equals(x.Key, item.Key));
            }
        ...
        public bool ICollection<KeyValuePair<TKey, TValue>>.Remove(KeyValuePair<TKey, TValue> item)
        {
            if (IsUsingList)
            {
                ...
                for (int ix = 0; ix < List.Count; ix++)
                {
                    if (Comparer.Equals(List[ix].Key, item.Key))
                    {
