    public static IEnumerable<FileEntryInfo> EnumerateFindFileSystemEntries(string entryToFind, StringComparison comparison = StringComparison.CurrentCultureIgnoreCase, string[] drivesToSearch = null)
    {
        if(drivesToSearch == null)
    ...
    private static IEnumerable<FileEntryInfo> EnumerateFindEntryRoot(string fileNameToFind, DirectoryInfo rootDir, StringComparison comparison = StringComparison.CurrentCultureIgnoreCase)
    {
        ...
        FileEntryInfo root = new FileEntryInfo(rootDir.FullName, 0);
        queue.Enqueue(root);
        while (queue.Count > 0)
        {
