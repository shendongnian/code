    private void FindRegions(int cx0, int cx1, int cy0, int cy1, int radius0, int radius1, ref Region rgnLeft, ref Region rgnRight)
        {
            ...
            Double dist = Math.Sqrt(dx * dx + dy * dy); //distance between pnt1 and pnt2
            ...
            fe = Math.Asin((-(souble)Math.Min(pnt2.Y, pnt1.Y) + (double)cy0) / (double)radius0);
            fe = (fe * 180D) / Math.PI;
            fe = (90 - fe) + 270;
            if (pnt2.Y >= pnt1.Y)
            {
                gp.AddLine(pnt1, pnt2);
            }
            else
            {
                gp.AddLine(pnt2, pnt1);
            }
