    ...
    public class RayCaster {
    	...
    	private void ProcessCollision(Collider current) {
    		...
    		if (current == null) {
    			// There was on object being hit last frame, but not this frame.
    			if (previous != null) {
    		...
    		// There was no object being collided last frame, just starting a new collision
    		else {
    			DoEvent(OnRayEnter, current);
    		}
		...
		public static int GetLayerMask(string layerName, int existingMask=0) {			int layer = LayerMask.NameToLayer(layerName);
