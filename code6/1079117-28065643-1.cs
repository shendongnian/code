    ...
    private string FindObject(Stack<object> self, Type typeToFind, string path)
    {
        ...
        foreach (var prop in _self.GetType().GetMembers().Where(x => !x.GetCustomAttributes(true).Any(y => y is XmlIgnoreAttribute || y is NonSerializedAttribute)))
        {
                ...
                case System.Reflection.MemberTypes.Event:
                    {
                        ...
                        if (field!=null && !TrySerialize(field.GetValue(_self)))
                        {
