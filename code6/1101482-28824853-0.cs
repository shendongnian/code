    ...
    {
       var joinQuery = attributesData.Where(x => x.attributeName == criterion.Key && x.strValue == criterion.Key);
       query = query.Join(joinQuery, x => x.strFormId, y => y.strFormId, (x, y) => x);
    }
    
    foreach(var criterion in likeSearchCriteria)
    {
       var joinQuery = attributesData.Where(x => x.attributeName == criterion.Key && x.strValue.Contains(criterion.Key));
       query = query.Join(joinQuery, x => x.strFormId, y => y.strFormId, (x, y) => x);
    }
