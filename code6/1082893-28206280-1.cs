        ...
        public class PolymorphicTypeConverter : JavaScriptConverter
        {
            ...
            Type FindUniqueTypeMatch(IDictionary<string, object> jsonProperties)
            {
                ...
                else
                {
                    ...
                    var candidates = AncestorsAndSelf(matches[0]).Reverse().ToList();
                    foreach (var match in matches.Skip(1))
                    {
                        candidates = candidates.Zip(AncestorsAndSelf(match).Reverse(), (t1, t2) => (t1 == t2 ? t1 : null)).Where(t => t != null).ToList();
                    }
