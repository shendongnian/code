    ...
    {
        int tagNumber = Convert.ToInt32(((Button)sender).Tag);
        
        string tbText = from x in this.Controls.OfType<TextBox>()
                    where x.Tag == tagNumber
                    select x.Text;
        string lblText = from x in this.Controls.OfType<Label>()
                    where x.Tag == tagNumber
                    select x.Text;
        MessageBox.Show(tbText + " " + lblText);
    }
