    public static class MsOfficeHelper
    {
        ...
        public static bool IsPasswordProtected(Stream stream)
        {
            ...
            const int defaultScanLength = 16384;
        ...
        static bool ScanForPassword(Stream stream, byte[] buffer, int sectionSize)
        {
            ...
            try
            {
                ...
                if (headerOffset >= 0)
                {
                    ...
                    short bof = BitConverter.ToInt16(workbookStream, 0);
                    short bofLength = BitConverter.ToInt16(workbookStream, 2);
                    const short bofMagic = 0x0809;
                    const short filePassMagic = 0x002F;
                    if (bof != bofMagic)
                        return false; // invalid BOF
                    short filePass = BitConverter.ToInt16(workbookStream, 4 + bofLength);
                    // check if we have a valid FILEPASS record
                    return filePass == filePassMagic;
                }
            }
            catch (Exception ex)
            {
