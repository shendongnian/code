    ...
    private static float ExifGpsToFloat(PropertyItem propItemRef, PropertyItem propItem)
    {
        ...
        uint degrees = degreesNumerator / (float)degreesDenominator;
        uint minutesNumerator = BitConverter.ToUInt32(propItem.Value, 8);
        uint minutesDenominator = BitConverter.ToUInt32(propItem.Value, 12);
        uint minutes = minutesNumerator / (float)minutesDenominator;
        uint secondsNumerator = BitConverter.ToUInt32(propItem.Value, 16);
        uint secondsDenominator = BitConverter.ToUInt32(propItem.Value, 20);
        uint seconds = secondsNumerator / (float)secondsDenominator;
