		...
		{
			OnErrorResumeNext(actions: actions);
		}
		/// <summary>
		/// Execute each of the specified action, and if the action is failed go and executes the next action.
		/// </summary>
		/// <param name="returnExceptions">if set to <c>true</c> return list of exceptions that were thrown by the actions..</param>
		...
		public static Exception[] OnErrorResumeNext(bool returnExceptions = false, bool putNullWhenNoExceptionIsThrown = false, params Action[] actions)
		{
			...
			foreach (var action in actions)
			{
				Exception exp = null;
				try { action.Invoke(); }
				catch (Exception ex) when (returnExceptions) { exp = ex; }
