        ...
        DataGridViewCellPaintingEventArgs e)
    {
        ...
        var sWrapped = e.Graphics.MeasureString(e.Value.ToString(), dataGridView1.Font,
            // Is is MeasureString that determines the height given the width, so that 
            // it properly takes the actual wrapping into account
        ...
        if (sOriginal.Width != dataGridView1.Columns[e.ColumnIndex].Width)
        {
                ...
                fontBrush = new SolidBrush(e.CellStyle.ForeColor))
            {
                ...
                // The DrawLine calls restore the missing borders: which borders are 
                // missing and how to paint them depends on the border style settings
                ...
                // it will later be set to the row height to avoid the flickering that
                // would occur by setting the height multiple times.
        ...
        {
            // Setting the height only in the last cell, when the full row has been painted, 
            // helps to avoid flickering when more than one row needs the wrap.
            dataGridView1.Rows[e.RowIndex].Height = (_rowMaxHeight > _rowDefaultHeight) 
