    private void Window_Closing(object sender, CancelEventArgs e)
    {
        ...
        // close.
        //
        // Note also that the OperationCanceledException that is thrown by 
        // cancelling OfflineExportTask is not handled. Nothing needed to be 
        // done, such as clean up, etc., plus it was unclear how to trap
        // for it when antecedent.Result -- which is where the exception would 
        // be raised -- doesn't work in this case since OfflineExportTask does 
        // not return a result. Plus, only the Close() call is executed,
        // so trying to trap for it would have necessitated writing extra code 
        // that wouldn't have performed
        // any additional operations anyway.
        #endregion
        if (OfflineExportTask != null
            && OfflineExportTask.Status != TaskStatus.RanToCompletion
            && OfflineExportTask.Status != TaskStatus.Canceled)
        {
            // Establish that that will run the moment the OfflineExportTask's 
