    public static class Factory<C> where C : IBaseClass, new()
    {
        ...
        public static void UnregisterHandlers(List<Action<object, SharedEventArgs>> list)
        {
            var toRemove = (from h in handlers where list.Contains(h.Value.Third) select h.Key).ToList();
            foreach (var key in toRemove) handlers.Remove(key);
        }
        ...
        public static void Destroy(C oldClass)
        {
            oldClass.UnRegister();
        }
