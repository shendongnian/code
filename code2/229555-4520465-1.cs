    ...
    string currUser = User.Identity.Name.ToString().Trim().Substring(User.Identity.Name.ToString().Trim().IndexOf("\\") + 1);
                    ...
                    Inventory.Employee.Details objEmpDetails = new Inventory.Employee.Details();
    
                    objEmpDetails = objEmpDB.Get(currUser);
    
                    Welcome.Text = "Current User: " + objEmpDetails.Employee_Full_Name;
    
                    Inventory.User.Details objUserDetails = new Inventory.User.Details();
                    ...
                    if (objUserDB.UserAuthenticates(currUser))
                    {
                        ...
                        if (objUserDetails.Active_User_Name != objUserDetails.User_Name)
                        {
                            ...
                            lServer.Text = "(" + objUserDB.UserPermissionName(objUserDetails.Active_Logon_Name) + ") - " + System.Environment.MachineName;
                            lShadow.ToolTip = Inventory.Properties.Settings.Default.connectionString.Substring(0,Inventory.Properties.Settings.Default.connectionString.IndexOf(';'));
