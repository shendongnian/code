    ...
    {    
        ...
        static StaticDynamicWrapper()
        {
            ...
            var funcTypes = Assembly.GetAssembly(typeof (Func<>)).GetTypes().Where(t => t.Name.StartsWith("Func`")).ToArray();
            int maxNoOfArgs = funcTypes.Max(t => t.GetGenericArguments().Length) - 3; //We need to subtract 3 from the absolute max to account for the return type and the 2 required parameter: callsite and target type.
                                        ...
                                        funcTypes
                                            .Where(
                                                f => f.GetGenericArguments()
                                                         .Count().Equals(funcGenericArguments.Count())
                                            ).Single()
