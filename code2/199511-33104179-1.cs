        ...
        class Node<Type> : LinkedList<Type> where Type : IComparable
        {   // Why inherit from LinkedList? A: We need to use polymorphism.
            public Type value;
            public Node(Type value) { this.value = value; }
        }
        class LinkedList<Type> where Type : IComparable
        {   // why constraint Type to be comparable? A: We need to insure no duplicated element.
            Node<Type> next;
            /// <summary> if x is in list, return previos pointer of x. 
