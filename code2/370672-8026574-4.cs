        ...
        else
        {
            ...
            else
            {
                ...
                if (!t.GetElementType().IsValueType)
                {
                    ...
                    while (Increase(indicies, lengths, p))
                    {
                        resultArray.SetValue(resultArray.GetValue(indicies).DeepClone(copies), indicies);
                    }
                }
    ...
    private static Boolean Increase(Int32[] indicies, Int32[] lengths, Int32 p)
    {
        if (p > -1)
        {
            indicies[p]++;
