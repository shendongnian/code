    ...
    // block the processor untill finishing executing [Sync calling]
    // Note: Don't worry about the time because there's a timer to check the timeout
    process.WaitForExit();
    
    // Disable the timer. because the process has finished executing.
    timer.Change(Timeout.Infinite, Timeout.Infinite);
    
    // if the process has finished by timeout [The timer which declared above] or finished normally [success or failed].
    if (execTimeout)
    {
        // Write in log here
    }
