    class Program
    {
        class ArraySegmentComparer : IEqualityComparer<ArraySegment<char>>
        {
            public bool Equals(ArraySegment<char> x, ArraySegment<char> y)
            {
                ...
                {
                    if (!x.Array[i].ToString().Equals(y.Array[i].ToString(), StringComparison.InvariantCultureIgnoreCase))
                    {
                        return false;
                    }
                }
            ...
            public int GetHashCode(ArraySegment<char> obj)
            {
                unchecked
                {
                    ...
                    if ((obj.Count & 1) != 0)
                    {
                        ...
                        hash *= 23;
                        hash += Char.ToUpperInvariant(obj.Array[end]);
                    }
                    int i;
                    for (i = obj.Offset; i < end; i += 2)
                    {
                        hash *= 23;
                        hash += Char.ToUpperInvariant(obj.Array[i]) << 16 + Char.ToUpperInvariant(obj.Array[i + 1]);
                    }
                    return hash;
                }
            }
        ...
        static void Main()
        {
            ...
            var sampleText = @"For my custom made chat screen i am using the code below for 
    checking censored words. But i wonder can this code performance improved. Thank you.
