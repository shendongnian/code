        ...
        public static int ReadData<T>(List<T> data, string table, T search, string connect) where T : class, new()
        {
            ...
            data.Clear(); 
            // Number of Ticks that equal 1800-01-01 in DateTime (is defined as null)
            long DateTimeNull = 567709344000000000; 
            ...
            foreach (PropertyInfo p in propinfs)
            {
                ...
                if (propvalue != null && dateticks != DateTimeNull) 
                    wherestr += wherestr == "" ? p.Name + " = @" + p.Name.ToLower() : ", " + p.Name + " = @" + p.Name.ToLower();
            }
            ...
            try
            {
                ...
                foreach (PropertyInfo p in propinfs) 
                {
                    ...
                    if (propvalue != null && !(propvalue.GetType() is DateTime && propvalue.Ticks != DateTimeNull))
