    ...
    boolean IsGoodPassword(string pwd){
        ...
        allowableChars = Regex.IsMatch(pwd, allowedCharsPattern , RegularExpressions.IgnoreCase));
        //Does it contain at least one # and one letter?
        oneLetterOneNumber = Regex.IsMatch(s, "[0-9]")) && Regex.IsMatch(s, "[a-z]", RegularExpressions.IgnoreCase));
