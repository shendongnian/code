    ...
    public static Rect CalculateBounds(Visual3D visual)
    {
        ...
        {
            bounds.Union(CalculateBounds(transform, modelVisual3D.Content));
    ...
    public static Rect CalculateBounds(GeneralTransform3DTo2D transform, Model3D model)
    {
        var region = Rect.Empty;
        var matrix = Matrix3D.Identity;
        if (model.Transform != null)
        {
            matrix.Append(model.Transform.Value);
        }
        ...
        else
        {
            ...
            if (model3DGroup != null)
            {
                ...
                {
                    region.Union(CalculateBounds(transform, child));
                }
            }
        }
        return region;
