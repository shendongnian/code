    ...
    {
        //Lets pretend our protocall sends a byte with 1 if the next object will be a Foo and a 2 if the next object will be a Bar, and 3 reads a int32.
            ...
            switch (buffer[0])
            {
                ...
                case 3:
                    ...
                    while (bytesRead < 4)
                    {
                        ...
                        bytesRead += await networkStream.ReadAsync(buffer, 1 + bytesRead, 4 - bytesRead);
                    }
                    int num = BitConverter.ToInt32(buffer, 1);
                    await DoSomethingWithANumberAsync(num);
                    
                    return;
                default:
                    await LogInvaidRequestTypeAwait(buffer[0]).ConfigureAwait(false);
                    return;
            }
        }
    }
