    ...
    foreach(KeyValuePair<string, List<string>> npc in NpcAI.RegisteredNpc)
    {
        using(SqlDatabaseClient client = SqlDatabaseManager.GetClient())
        {
            ...
            client.SetParameter("id", (object) npc.Value[0]); // will get the first value so it'll be 1 or 2
            client.SetParameter("name", (object) npc.Value[1]; // will get the second value, so it'll be either "Ballons" or "lons"     
