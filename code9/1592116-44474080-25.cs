    ...
    namespace ConcoxServer
    {
        ...
        public class GPS
        {
                    ...
                    while (true)
                    {
                        ...
                        while (fifo.Count > 0)
                        {
                            ...
                            while (true)
                            {
                                ...
                                //remove message from working buffer
                                //byteState = new KeyValuePair<List<byte>, StateObject>(byteState.Key.Skip(status.Value.Count()).ToList(), byteState.Value);
            ...
            static KeyValuePair<UNPACK_STATUS, byte[]> Unpack(List<byte> working_buffer)
            {
                ...
                return new KeyValuePair<UNPACK_STATUS, byte[]>(UNPACK_STATUS.GOOD_MESSAGE, packet);
            }
