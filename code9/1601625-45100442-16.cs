    ...
    public class BitmapHandler
    {
        ...
        public static Bitmap CloneImage(Bitmap sourceImage)
        {
            ...
            Int32 newDataWidth = ((Image.GetPixelFormatSize(sourceImage.PixelFormat) * rect.Width) + 7) / 8;
            CopyMemory(targetData.Scan0, sourceData.Scan0, sourceData.Stride * sourceData.Height, sourceData.Stride, targetData.Stride, newDataWidth);
        ...
        private static void CopyMemory(IntPtr target, IntPtr source, Int32 length, Int32 origStride, Int32 targetStride, Int32 copyLength)
        {
            IntPtr sourcePos = source;
            IntPtr destPos = target;
            Int32 copySize = Math.Min(Math.Min(origStride, copyLength), targetStride);
            Byte[] imageData = new Byte[copySize];
            
            while (length >= targetStride)
            {
                Marshal.Copy(sourcePos, imageData, 0, copySize);
                Marshal.Copy(imageData, 0, destPos, copySize);
                length -= origStride;
            ...
            if (length > 0)
            {
                length = Math.Min(length, copySize);
