    ...
    namespace ConsoleApplication50
    {
        class Program
        {
            static void Main(string[] args)
            {
                ...
                dt = dt.AsEnumerable().OrderBy(x => x, new SortRecord()).CopyToDataTable();
            }    
        }
        public class SortRecord : IComparer<DataRow> 
        {
            ...
            {
                string[] rowSplitArrayA = rowA.Field<string>(0).Split(new char[] { '-' });
                string[] rowSplitArrayB = rowB.Field<string>(0).Split(new char[] { '-' });
