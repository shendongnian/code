        public static void FindSpecialProperties(PropertyInfo property, List<PropertyInfo> allProperties, List<int> recursivedProperties = null)
        {
            if (recursivedProperties == null)
            {
                recursivedProperties = new List<int>();
            }
            if (recursivedProperties.Contains(property.GetHashCode()))//Eliminate already recursived property
            {
                return;
            }
            recursivedProperties.Add(propertyInfo.GetHashCode());
