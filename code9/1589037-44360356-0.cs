    		if (textBox1.Text != "")
		{
			...
			bool valid_month = new Regex(@"[0][1-9]|[1][0-2]").IsMatch(sTown) && new Regex(@"[^a-z]", RegexOptions.IgnoreCase).IsMatch(sTown) && sMonth.Length ==2;
            int year = 0;
            bool isNum = int.TryParse(sYear,out year);
            bool valid_year = new Regex(@"[^a-z]", RegexOptions.IgnoreCase).IsMatch(sTown) && sYear.Length == 4 && (isNum ? year > 1980 && year < 2100 : false);
