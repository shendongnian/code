    public class Editor : TextBox
    {
        ...
        public Editor(Label RefLabel, string WordToMatch)
        {
            ...
            this.Enter += (sender, e) => { this.Font = this.UnderlineFont; };
        ...
        {
            return TextRenderer.MeasureText(_Label.CreateGraphics(), _mask, _Label.Font, this._Label.Size, _flags);
        }
        private string CreateMask(int _EditorWidth)
        {
            ...
            if (GetTextSize(_TestMask).Width <= (_EditorWidth))
            {
                SubstChar = SubstCharLarge;
            } else {
                SubstChar = SubstCharSmall;
                _TestMask = new StringBuilder().Insert(0, SubstCharSmall, this.MatchWord.Length).ToString();
            }
            return _TestMask;
        }
    }
