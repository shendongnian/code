    public List<string> GetIntervals(DateTime startDate, DateTime endDate) {
        ...
        DateTime target = startDate;
   	    while (target <= endDate) {
    	    ...
    	    if (!result.Contains(date.ToShortDateString())) {
                result.Add(date.ToShortDateString());
            }
	        target = target.AddMonths(1);
   	    }
        if (!result.Contains(startDate.ToShortDateString())) {
            result.Add(startDate.ToShortDateString());
        }
        if (!result.Contains(endDate.ToShortDateString())) {
