    ...
    namespace MyDataflow {
        class MyDataflow {
            ...
            public void HndlingCompletion() {
                ...
                batchBlock.Completion.ContinueWith(batchBlockCompletionTask => {
                    if (!batchBlockCompletionTask.IsFaulted) {
                        xForm1.Complete();
                        xForm2.Complete();
                    }else {
                        ((IDataflowBlock)xForm1).Fault(batchBlockCompletionTask.Exception);
                        ((IDataflowBlock)xForm2).Fault(batchBlockCompletionTask.Exception);
                    }
