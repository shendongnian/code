    ...
    namespace ConcoxServer
    {
        public class StateObject
        {
            ...
            public static List<byte> working_buffer { get; set; }
        ...
        public class GPS
        {
            ...
            static byte[] alarmResponse = { 0x78, 0x78, 0x85, 0x97, 0x7E, 
                                              0x00, 0x00, 0x00, 0x01,
            ...
            public static List<KeyValuePair<UNPACK_STATUS, List<byte>>> history = new List<KeyValuePair<UNPACK_STATUS, List<byte>>>();
            ...
            public void StartListening(string IP, int port, Boolean test)
            {
                ...
                try
                {
                    ...
                    List<byte> working_buffer = new List<byte>();
                    while (true)
                    {
                        allDone.WaitOne();
                        working_buffer.AddRange(StateObject.working_buffer);
                        StateObject.working_buffer.Clear();
                        while(true)
                        {
                            UNPACK_STATUS status =  Unpack(ref working_buffer);
                            if (status == UNPACK_STATUS.NOT_ENOUGH_BYTES) break;
                            ...
                            else
                            {
                                ...
                                switch (protocolNumber)
                                {
                                    case PROTOCOL_NUMBER.LOGIN_MESSAGE:
                                        ...
                                        Send(StateObject.workSocket, loginResponse);
                                        break;
                                    ...
                                    case PROTOCOL_NUMBER.ALARM_DATA:
                                        ...
                                        Send(StateObject.workSocket, alarmResponse);
                                        break;
                                } //end switch
                            }// End if
                        } //end while
            ...
            static UNPACK_STATUS Unpack(ref List<byte> working_buffer)
            {
                //return null indicates an error
                if (working_buffer.Count() < 3) return UNPACK_STATUS.NOT_ENOUGH_BYTES;
                int len = working_buffer[2];
                if (working_buffer.Count < len + 5) return UNPACK_STATUS.NOT_ENOUGH_BYTES;
                // check start and end bytes
                if ((working_buffer[0] != 0x78) && (working_buffer[1] != 0x78) && (working_buffer[len + 3] != 0x0D) && (working_buffer[len + 4] != 0x0A))
                {
                    return UNPACK_STATUS.ERROR;
                }
                byte[] packet = working_buffer.Take(len + 5).ToArray();
                ...
                {
                    history.Add(new KeyValuePair<UNPACK_STATUS, List<byte>>(UNPACK_STATUS.ERROR, packet.ToList()));
                    return UNPACK_STATUS.ERROR;
                }
                history.Add(new KeyValuePair<UNPACK_STATUS, List<byte>>(UNPACK_STATUS.GOOD_MESSAGE, packet.ToList()));
                return UNPACK_STATUS.GOOD_MESSAGE;
            }
             ...
             public static void AcceptCallback(IAsyncResult ar)
            {
                ...
                StateObject.working_buffer = new List<byte>();
            ...
            public static void ReadCallback(IAsyncResult ar)
            {
                ...
                if (bytesReads > 0)
                {
                    StateObject.working_buffer.AddRange(state.buffer.Take(bytesReads).ToArray());
                    //UNPACK_STATUS status = UNPACK_STATUS.DEFAULT;
                    //do
                    //{
                    //    status = Unpack(state);
                    //    switch (status)
                    //    {
                    //        case UNPACK_STATUS.ERROR:
                    //            StateObject.status = false;
                    //            break;
                    //        case UNPACK_STATUS.GOOD_MESSAGE:
                    //            StateObject.messageCount++;
                    //            StateObject.status = true;
                    //            break;
                    //        case UNPACK_STATUS.NOT_ENOUGH_BYTES:
                    //            break;
                    //    }
                    //} while (status == UNPACK_STATUS.GOOD_MESSAGE);
                }
                handler.BeginReceive(state.buffer, 0, BUFFER_SIZE, 0,
                    new AsyncCallback(ReadCallback), state);
