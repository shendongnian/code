           static void Main(string[] args)
            {
                ...
                var results = ldt.OrderBy(x => x.name).Select((x, i) => new { person = x, index = i })
                    ...
                    .Select(x => x.GroupBy(y => y.index % ROWS_PER_PAGE).Select(z => new { column1 = z.FirstOrDefault().person.name, column2 = z.Skip(1).FirstOrDefault().person.name, column3 = z.LastOrDefault().person.name }))
                    .SelectMany(x => x);
            }
        }
