    public class Expresion {
        ...
        public void CreateExpression() {
            ...
            var enumerableSelectIndexTSourceTResult = (from x in typeof(Enumerable).GetMethods()
                                                        ...
                                                        where pars.Length == 2
                                                        let fn = pars[1]
                                                        where fn.ParameterType == typeof(Func<,,>).MakeGenericType(args[0], typeof(int), args[1])
                                                        select x).Single();
            ...
            var innerFunction1 = Expression.Constant(innerExpression1.Compile());
            // .Select((item, index) => new { item, index })
            var select1 = Expression.Call(enumerableSelectIndex, getItem, innerFunction1);
            // Inner function end
            // Enumerable.Select without indexer (generic)
            var enumerableSelectTSourceTResult = (from x in typeof(Enumerable).GetMethods()
                                                    where x.Name == "Select" && x.IsGenericMethod
                                                    let args = x.GetGenericArguments()
                                                    where args.Length == 2
                                                    let pars = x.GetParameters()
                                                    where pars.Length == 2
                                                    let fn = pars[1]
                                                    where fn.ParameterType == typeof(Func<,>).MakeGenericType(args)
                                                    select x).Single();
            ...
            var innerFunction2 = Expression.Constant(innerExpression2.Compile());
            // Inner function end
            // .Select((previous select), x => x.index)
            var select2 = Expression.Call(enumerableSelect, select1, innerFunction2);
            // Enumerable.ToList (generic)
            var enumerableToListTSource = (from x in typeof(Enumerable).GetMethods()
