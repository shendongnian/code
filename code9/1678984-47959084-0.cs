    ...
    for (int i = 0; i < alreadyAnsweredQues.Count; i++)
    {
        ...
        //cmd.Parameters.AddWithValue(parameters[i].ToString(), alreadyAnsweredQues[i]);
        
        cmd.Parameters.Add(parameters[i].ToString(), SqlDbType.VarChar,System.Int32.MaxValue).Value = alreadyAnsweredQues[i];
    }
    //cmd.Parameters.AddWithValue("@cid", Session["c_id"].ToString());
