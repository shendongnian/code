    ...
    [DllImport("user32.dll")] static extern int SendMessage(int  hWnd, int wMsg, int wParam, int lParam);
        ...
        public static void killExcelMbox(Object windowTitle)
        {   for (int h = 0; h == 0;)
            {   Thread.Sleep(1000);
        ...
        {  Thread mboxKiller = new Thread(killExcelMbox);
           // Excel message-boxes usually have this window title. Change if
           // your message-box has a different title
