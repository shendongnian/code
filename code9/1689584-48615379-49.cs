    public class Quiz : IDisposable
    {
        ...
        private void Editors_Setup()
        {
            ...
            {
                List<Point> _Positions = GetEditorsPosition(this.Label, _word);
        ...
        private List<Point> GetEditorsPosition(Label _label, QuizWord _word)
        {
            List<int> _idxes = Regex.Matches(_label.Text, _word.WordMask)
                                    .Cast<Match>()
                                    .Select(t => t.Index).ToList();
            return _idxes.Select(idx => this._Multiline.GetPositionFromCharIndex(idx)).ToList();
        }
