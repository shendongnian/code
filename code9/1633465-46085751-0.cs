        ...
        class Program
        {
            static void Main(string[] args)
            {
                ...
                if (useSampleDate) {  // test without using the console input
                    ...
                    for (int studentId = 0; studentId < TotalStudents; studentId++) {
                        ...
                        for (int testId = 0; testId < TotalGrades; testId++) {
                            Console.Write("Please enter exam score {0} for student {1}: ", testId + 1, name);
                            gradeBook[name].Add(Convert.ToDouble(Console.ReadLine()));
                        }
                    }
                ...
                foreach (var student in gradeBook) {
                    Console.WriteLine("Student " + student.Key + " scored an average of " + student.Value.Average() + ". " + student.Key +
                        " will recieve a(n) " + gradeTranslator[(int)(student.Value.Average() / 10)] + " in the class.\n");
                }
