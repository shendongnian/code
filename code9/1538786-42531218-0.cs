    public IEnumerable<VendorDto> GetVendors(string sourceLanguage = null, string targetLanguage = null, string service = null)
            {
                var vendorsQuery = _context.Vendors
    				...
    				Expression<Func<Translation, bool>> targetLanguagePredicate = z => z.TargetLanguage.Name == sourceLanguage;
    				Expression<Func<Translation, bool>> servicePredicate = z => z.Service.Name == sourceLanguage;
    				...
    				if(hasSource)
    				{
    					vendorsQuery = vendorsQuery.Where(x => x.Translations.Any(sourceLanguagePredicate));
    				}
    				
    				if(hasTarget)
    				{
    					vendorsQuery = vendorsQuery.Where(x => x.Translations.Any(targetLanguagePredicate));
    				}
    				
    				if(hasService)
    				{
    					vendorsQuery = vendorsQuery.Where(x => x.Translations.Any(servicePredicate));
    				}
