    ...
    // Convenient overload for GetNextDate which calls it with today's date, so that you don't have to specify.
    static DateTime GetNextDate(int day, bool includeCurrentDay = false)
    {
        return GetNextDate(day, DateTime.Now, includeCurrentDay);
    }
    // Get a DateTime which represents the next time a day of number $day comes up in the calendar
    static DateTime GetNextDate(int day, DateTime cDate, bool includeCurrentDay = false)
    {
        int cDay = cDate.Day;
