    private static void Main(string[] args)
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                {
                    sb.Append(GetKeyValueJson($"Thing {i} ID", fields[i]));
                    sb.Append(GetKeyValueJson($"Thing {i + 1} ID", fields[i + 1]));
                    sb.Append(i + 3 == fields.Length
                        ? GetKeyValueJson($"Thing {i + 2} ID", fields[i + 2], true)
                        : GetKeyValueJson($"Thing {i + 2} ID", fields[i + 2]));
                }
