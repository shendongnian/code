    public partial class ButtonDragging
    {
        ...
        private void BuildButtonGrid()
        {
            ...
            for (var i = 0; i < rows; i++)
            {
                for (var j = 0; j < columns; j++)
                {
                    ...
                    button.PreviewMouseMove += this.OnButtonMouseMove;
                    button.PreviewMouseLeftButtonDown += this.OnButtonLeftButtonDown;
                    button.PreviewMouseLeftButtonUp += this.OnButtonLeftButtonUp;
                    button.Drop += this.OnButtonDrop;
                    button.Click += this.OnButtonClick;
                    button.LostMouseCapture += this.OnButtonLostMouseCapture;
        ...
        {
            _mouseDownButton?.ReleaseMouseCapture();
        ...
        private void OnButtonMouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (Math.Abs(distance.X) > SystemParameters.MinimumHorizontalDragDistance &&
                Math.Abs(distance.Y) > SystemParameters.MinimumHorizontalDragDistance)
            {
                var button = (Button)sender;
