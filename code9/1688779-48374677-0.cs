The problem is you're comparing the texture subrects, not the object's bounds. Projectile.logRect and Wood_Level_1.rectangle are the texture subrects, which you use for both collision detection and drawing. You need to store and update the current positions of each object so you can make comparisons using them.
    class Projectile
    {
        //...
        ...
        public void Update(GameTime gameTime)
        {
            ...
            var vpWidth = ISU.graphics.GraphicsDevice.Viewport.Width;
            var vpHeight = ISU.graphics.GraphicsDevice.Viewport.Height;
            if (keyState == 1)
            {
                position.Y = (float)(vy * t2 + g * t2 * t2 / 2) + 540 - texture.Height;
                position.X = (float)((vx * -1) * t2) + 60;
                t2 = t2 + gameTime.ElapsedGameTime.TotalSeconds;
            }
            // Compute this once.
            var bottom = vpHeight - texture.Height;
            ...
            // Compute this once.
            var right = vpWidth - texture.Width;
            if (position.X > right)
            {
                position.X = right;
                keyState = 0;
