        ...
        private class MultilineSupport
        {
            ...
            private void Initialize(Label label)
            {
                ...
                foreach (string _word in _wordslist)
                {
                    ...
                    int _sublength = MeasureString((_substring + _word + 
                                                    (_currentlistindex < _wordslist.Count 
                                                    ? ((char)32).ToString() 
                                                    : string.Empty)));
            ...
            private Point CalcPosition(int Line, int Index)
            {
                ...
                int _horizontalzpos = MeasureString(this.Lines[Line]
                                      .Substring(0, Index - GetFirstCharIndexFromLine(Line)));
                return new Point(_horizontalzpos + _font_padding, _verticalpos);
            }
