    private static void Main()
    {
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            foreach (var divisibleByInt in divisibleByInts)
            {
                ...
                {
                    // This random number is divisible by one of the numbers 
                    // we care about, so add or update the appropriate list 
                    // in the dictionary at that key with this value
        ...
        foreach (var divisibleByInt in divisibleByInts)
        {
            var countOfThisItem = (values.ContainsKey(divisibleByInt))
                ? values[divisibleByInt].Count
                : 0;
