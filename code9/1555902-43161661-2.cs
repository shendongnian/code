    static void Main()
    {
        ...
        // initialize arrays of available number
        for (int i = 0; i < 9; i++)
        {
        ...
        for (int row = 0; row < 9; row++)
        {
            ...
            for (int column = 0; column < 9; column++)
            {
                ...
                var availableItems = rows[row].Intersect(columns[column].Intersect(blocks[block])).ToList();
                // If we reach a point where there are no availableItems, then this is
                // not a valid Sudoku pattern. Reset everything and try again (brute force method).
                ...
                // Update our map with this item (add one so it's 1-9)
