    ...
    var index = UserManager.Users.Select((user, index) => new {user,index}).OrderBy(a=>a.user.UserName).Where(a=>a.user.UserName == "_newUserName").Select(a=>a.index).FirstOrDefault();
