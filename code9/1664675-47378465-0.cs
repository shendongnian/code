    ...
    public static class LinqExtensions
    {
    	...
    	{
            // make a list of the selectors to make iteration easier
    		List<Func<TSource, TKey>> keySelectorsList = new List<Func<TSource, TKey>>(keySelectors);
    		return source.NestedGroupBy(factory, keySelectorsList, 0);
    	}
    
    	private static IEnumerable<TTarget> NestedGroupBy<TSource, TTarget, TKey>(this IEnumerable<TSource> source, Func<TKey, IEnumerable<TTarget>, TTarget> factory, List<Func<TSource, TKey>> keySelectors, int selectorIndex)
    	{
            // reached the end, just return an empty list
    		if(selectorIndex >= keySelectors.Count)
    		{
    ...
    public static void Main()
        ...
        */
        ...
        var groupings = result.NestedGroupBy<LevelObject, SomeObject, string>((key, children) =>
        	new SomeObject(key, children),
