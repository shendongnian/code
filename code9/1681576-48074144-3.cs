    namespace PipesAsyncAwait471
    {
        ...
        internal class Program
        {
            private static async Task Main()
            {
                ...
                tasks.AddRange(Enumerable.Range(0, 1000).Select(i => SendRequestAsync(i, 0, 100)));
                ...
                try {
                    ...
                    if (server.IsConnected) {
                        if (server.CanRead) {
                            ...
                            id = BitConverter.ToInt32(request, 4);
                            Console.WriteLine($"{index}:{id}:3 Client ping reached the server.");
                        }
                        if (index > -1 && server.CanWrite) {
                            var response = new[] { index, id }.SelectMany(BitConverter.GetBytes).ToArray();
            ...
            private static async Task SendRequestAsync(int index, int id, byte max)
            {
                ...
                try {
                    ...
                    if (client.IsConnected) {
                        ...
                        if (client.CanWrite) {
                            var request = new[] { index, id }.SelectMany(BitConverter.GetBytes).ToArray();
