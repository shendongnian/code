                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            string terminalID = Encoding.ASCII.GetString(loginMessage.Skip(4).Take(messageLength - 5).ToArray());
                            ...
                            {
                                allDone.WaitOne();
                                byte[] locationMessage = StateObject.history.Last().ToArray();
                                messageLength = locationMessage[2];
                                //message is 2 start bytes + 1 byte (message length) + 1 byte message length + 2 end bytes
                                Console.WriteLine("Status : '{0}', Receive Message : '{1}'", StateObject.status ? "Good" : "Bad", BytesToString(locationMessage.Take(messageLength + 5).ToArray()));
                                allDone.Reset();
                            }
                        }
                    }
