           public GPS(string IP, int port, Boolean test)
            {
                try
                {
                    ...
                    cmdLocation.Parameters.Add("@Longitude", SqlDbType.Float);
                    cmdLocation.Parameters.Add("@Lattitude", SqlDbType.Float);
                        ...
                        while((row = ReadWriteFifo(Mode.READ, null)) != null)
                        {
                            switch(row.message)
                            {
                                ...
                                case DATABASE_MESSAGE_TYPE.LOCATION:
                                    cmdLogin.Parameters["@IMEI"].Value = ((WriteDBMessageLocation)row).IMEI;
                                    cmdLogin.Parameters["@TrackTime"].Value = ((WriteDBMessageLocation)row).trackTime;
                                    cmdLogin.Parameters["@currTime"].Value = ((WriteDBMessageLocation)row).currTime;
                                    ...
                                    longStr = string.Format("{0}º{1}", degrees, minutes);
                                    cmdLogin.Parameters["@Longitude"].Value = ((WriteDBMessageLocation)row).longitude;
                                    ...
                                    latStr = string.Format("{0}º{1}", degrees, minutes);
                                    cmdLogin.Parameters["@Lattitude"].Value = ((WriteDBMessageLocation)row).lattitude;
                                    cmdLogin.Parameters["@speed"].Value = ((WriteDBMessageLocation)row).speed;
