            ...
            */
            ...
            for (int i=0; i<xrefs.Count; i++)
            {
                ...
                for (int j=i+1; j<xrefs.Count; j++) //we check with the other xrefs to see if they follow immediately after.
                {
                    ...
                    regCompare += "([ ]|(, )|())" + Regex.Escape(xrefs[j].OuterXml); //we check that the the xref comes exactly after a space or a comma and space. The xml reader wont read the whitespace between two tags.
