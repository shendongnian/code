    public void CopyImport2CT(string myTable)
    {
        ...
        try
        {
            ...
            {
                string copystring = "INSERT INTO Landentabel (LandenID, Landnummer, Landnaam, Piektarief, Daltarief, Weekendtarief) SELECT (SELECT COUNT(*) FROM " + myTable + " WHERE e.CountryCode >= CountryCode) AS Id, e.CountryCode, e.Country, e.HighFee, e.LowFee, e.Weekendfee from " + myTable + " e";
