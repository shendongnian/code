	...
	{
		private static readonly ILog _log = LogManager.GetLogger("ShiftWindowOntoScreen");
		/// <summary>
		///		Intent:  
		///		- Ensure that the window is on the visible screen.
		///		- Ensure that the window is not overlapping the task bar.
		/// </summary>
		public static void ShiftWindowOntoScreen(Window window)
		{
			_log.Info("Shifting notification window onto the screen.");
			...
			// Shift window away from taskbar.
			{
				...
				foreach (var taskBar in taskBarLocation)
				{
					...
					while (windowRect.IntersectsWith(taskBar))
					{
						...
						// Our window is covering the task bar. Shift it up. 
