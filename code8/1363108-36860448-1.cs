    ...
    {
        // uses inner join to find possible matches with the from and to with the currently available flights/routes
        ...
        var desiredRoute = routes.FirstOrDefault(r => r.From == from && r.To == to);
        if (desiredRoute != null)
        {
            return desiredRoute.Flights;
        }
