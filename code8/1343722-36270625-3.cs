        ...
        private static object getGroupedData(DataTable dt)
        {
            ...
            var results = products
              .Select(p => new //need a partial result first, containing the Global, Apac and Emea rows, if they exist
              {
                  ...
                  ParentId = products.FirstOrDefault(par => par.Name == p.Parent && par.Level + 1 == p.Level)?.Id,
              ...
              .Select(x => new //create the final result
              {
                  ...
                  Name = x.Name,
                  ParentId = x.ParentId,
                  GlobalValue1 = x.Global?.Field<double?>("Value1"),
                  GlobalValue2 = x.Global?.Field<double?>("Value2"),
                  APACValue1 = x.Apac?.Field<double?>("Value1"),
                  APACValue2 = x.Apac?.Field<double?>("Value2"),
                  EMEAValue1 = x.Emea?.Field<double?>("Value1"),
                  EMEAValue2 = x.Emea?.Field<double?>("Value2"),
              })
              .ToArray();
            return results;
        }
