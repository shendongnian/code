    ...
    public static class PersonEmployeeListSerializer
    {
        static XmlSerializer instance;
        static PersonEmployeeListSerializer()
        {
            using (Person.PushXmlTypeName("Employee"))
            {
                var lOverrides = new XmlAttributeOverrides();
                //var lAttributes = new XmlAttributes();
                //lOverrides.Add(typeof(Person), lAttributes);
                instance = new XmlSerializer(typeof(List<Person>), lOverrides, new Type[0], new XmlRootAttribute("Employees"), null);
            }
        }
