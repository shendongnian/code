	...
	var currentMonth = 
		...
		join last in resourcesLastMonth on new { last.SubscriptionId, last.ItemId } equals new { current.SubscriptionId, current.ItemId } into outer
	...
	var lastMonth = 
		...
		select new Resource
		{
			...
			EffectiveRate = current?.EffectiveRate ?? last.EffectiveRate,
			ResourceName = current?.ResourceName ?? last.ResourceName,
			UnitOfMeasure = current?.UnitOfMeasure ?? last.UnitOfMeasure
		});
