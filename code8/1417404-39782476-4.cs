    // Needed to generate model type names like "Collection of {MyType}" instead of "Collection of Object".
    if (!modelType.IsGenericType                                // Model type must not be a generic type.
        ...
        && typeof(IList).IsAssignableFrom(modelType.BaseType))  // Model type base type must implement the IList interface (you can replace "IList" by "IEnumerable" to handle different types of collections).
    {
