    public async Task<string> GetFilePathAsync(TestModel model)
        {
            ...
            {                   
                ...
                filePath = await cgRequestHelper.SaveCgStreamAsync(cgStream , serviceModel.FileName);
            }
            return filePath;
        }
    
        public async Task<string> SaveCgStreamAsync(Stream cgStream, string fileName)
            {
                if (response == null)
