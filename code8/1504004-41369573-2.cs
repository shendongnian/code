    bool Test(int input)
    {
        var foundPair = new Dictionary<Tuple<int, int>, Action<Tuple<int, int>, int>>
        {
        ...
        // found
        if(foundPair != default(KeyValuePair<Tuple<int, int>, Action<Tuple<int, int>, int>>)
        {
            foundPair.Value(foundPair.Key, input);
        }
        else
        {
    ...
    void Method1(Tuple<int, int> range, int input)
    {
        return true;
    }
