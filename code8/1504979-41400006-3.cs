    WITH genDates  
    AS (
        SELECT CAST('2016-01-01' AS DATETIME) AS mdate
        UNION ALL
        SELECT DATEADD(week,1,mdate) FROM genDates WHERE DATEADD(week,1,mdate) < CAST('2018-01-01' AS DATETIME)
    )
    SELECT mdate AS StartDate, DATEADD(day,6,mdate) AS EndDate, DATEPART(week, mdate) AS WeekOfYear
    FROM genDates OPTION (MAXRECURSION 200);
    +---------------------+---------------------+------+
