    public class Cached<FromT, ToT>
    {
        ...
        public Cached (Func<FromT, Task<ToT>> _GetSomethingTheLongWayAsync) {
            GetSomethingTheLongWayAsync = _GetSomethingTheLongWayAsync;
    }
        const int Expiration = 60;
        private static readonly ConcurrentDictionary<FromT, SemaphoreSlim> _keyLocks = new ConcurrentDictionary<FromT, SemaphoreSlim>();
        private static readonly ConcurrentDictionary<FromT, Tuple<ToT, DateTime>> _cache = new ConcurrentDictionary<FromT, Tuple<ToT, DateTime>>();
        private static bool IsExpiredDelete(Tuple<ToT, DateTime> value, FromT key)
        {
