    ...
    public class Validator<TAttribute> where TAttribute : Attribute
    {
        ...
        public bool Validate<TTarget>(TTarget target, IList<string> failedValidationsBag)
        {
            var valid = true;
            ...
            foreach (var p in properties)
            {
                ...
                if (!rules.TryGetValue(p.PropertyType, out predicate))  //check if there is an exact type match
                {
                    ...
                    if (assignableTypes.Any())
                    {
                        if (assignableTypes.Count > 1)
                        {
                            ...
                            while (assignableTypes.Count > 1)
                            {
                                ...
                                if (assignableTypes.Count == count) //We haven't made progress. Bail out.
