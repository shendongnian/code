           public string GetAssetRegisterDataTableBySp(ParameterDto parameters)
        {
            ...
            //AutoMapper.Mapper.CreateMap<Arhata.Assetrak.Entity.AssetRegisterReport, Arhata.Assetrak.Dto.ReportsDto>()
            //                            .ForMember(dest => dest.ParentAssetBarcode, opt => opt.Ignore())
            //                .ForMember(dest => dest.ParentAssetLatestStatus, opt => opt.Ignore())
            //                .ForMember(dest => dest.LastProject, opt => opt.Ignore())
            //                .ForMember(dest => dest.LastStatus, opt => opt.Ignore());
            //var reportDto = AutoMapper.Mapper.Map<IEnumerable<AssetRegisterReport>, IEnumerable<Arhata.Assetrak.Dto.ReportsDto>>(taggingReport);
            taggingReport.RemotingFormat = SerializationFormat.Xml;
