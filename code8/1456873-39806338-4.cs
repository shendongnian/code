                    ...
                    if (!reader.EOF)
                    {
                        ...
                        newNetwork.subnetworks = XNetwork.Elements().Where(x => x.Name.LocalName == "SubNetwork").Select(x => new SubNetwork()
                        {
                            ...
                            sites = x.Descendants().Where(y => y.Name.LocalName == "MeContext").Select(y => new Sites() {
                                ...
                                cellName = y.Descendants().Where(z => (z.Name.LocalName == "VsDataContainer") && (z.Descendants().Where(a => (a.Name.LocalName == "vsDataType") && ((string)a == "vsDataEUtranCellFDD")).Any())).Select(z => new CellName() {
