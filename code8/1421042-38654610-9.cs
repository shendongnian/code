    ...
    namespace ConsoleApplication2
    {
        class Program
        {
            ...
            static IEnumerable<IEnumerable<T>> permute<T>(IEnumerable<T> sequence, int count)
            {
                ...
                foreach (var startingElement in sequence)
                    foreach (var permutationOfRemainder in count == 1 ? new[]{new T[0]} : permute(allExcept(sequence, startingElementIndex++), count-1))
