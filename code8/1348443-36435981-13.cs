        ...
        string bodyStringJSON = Encoding.Default.GetString(bodyBuffer.ToArray());
        bodyBuffer.Close(); 
        //Response.Write(Server.HtmlEncode(bodyStringJSON));
        // REVERSE: the same serializer
        bodyBuffer = new MemoryStream(Encoding.Unicode.GetBytes(bodyStringJSON));
