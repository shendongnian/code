    ...
    public class SettingService : Service<Setting>, ISettingService
    {
        ...
        public T LoadSetting<T>() where T : ISetting, new()
        {
            ...
            var ayars = GetAllSettingsCached().Where(x => x.Name.StartsWith(settingName)).ToList();
            foreach (var propertyInfo in properties)
            {
                ...
                if (q != null)
                {
                    propertyInfo.SetValue(setting, Convert.ChangeType(q.Value, propertyInfo.PropertyType), null);
                }
            }
            return setting;
        }
