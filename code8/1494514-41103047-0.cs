    ...
    public class JsonStringWrapperConverter : JsonConverter
    {
        ...
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            ...
            {
                writer.WriteRawValue(((JsonStringWrapper)value).theString);
            }
        }
    ...
    protected JsonResult Json(string message, object data)
    {
        ...
        // Where it matters, be more careful about deciding
        //   whether or not the string is really JSON.
        if (data is string
         && (new char[] { '[', '{' }.Contains(((string)data).First())))
        {
            output = new PreSerializedJsonResult();
            finalData = new JsonStringWrapper(data as string);
        }
        output.Data = new
        {
