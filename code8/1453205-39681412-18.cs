    ...
    public class Validator<TAttribute> where TAttribute : Attribute
    {
        ...
        public bool Validate<TTarget>(TTarget target, IList<string> failedValidationsBag, bool onlyExactTypeMatchRules = false)
        {
            ...
            foreach (var p in properties)
            {
                ...
                //2. If onlyExactMatchRules is true no rule is chosen and we are finished.
        ...
        private bool checkRule<T>(T target, Type propertyType, object value, Predicate<object> predicate, string propertyName, IList<string> failedValidationsBag)
        {
            if (predicate != null && !predicate(value))
            {
                failedValidationsBag.Add($"{target}: {propertyName} failed validation. [Rule: {propertyType.Name}]");
                return false;
            }
            return true;
        }
