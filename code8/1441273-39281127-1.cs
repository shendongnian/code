	...
	class Program
	{
		static void Main(string[] args)
		{
			...
			foreach(var transaction in transactions)
			{
				...
				int daysUntilEndOfWeek_Friday = ((int)DayOfWeek.Saturday - (int)transactionDayOfWeek.DayOfWeek + 7) % 7;
				transaction.Newly_Added_Property_To_Group_By_Week_To_Get_Averages = transactionDayOfWeek.AddDays(daysUntilEndOfWeek_Friday).ToShortDateString();
				//Console.WriteLine("{0} {")
			}
