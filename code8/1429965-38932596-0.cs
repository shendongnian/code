    ...
    namespace ConsoleApplication7
    {
        ...
        public class GPS
        {
            ...
            public GPS(byte[] bytes)
            {
                ...
                sUerID = bytes.Skip(22).Take(12).ToArray();
                sDateTime = bytes.Skip(34).ToArray();
            }
            public double ToFloat(byte[] bytes)
            {
                return bytes[0] + (bytes[1] / 256) + (BitConverter.ToUInt16(bytes, 2)/ 65536);
            }
        }
