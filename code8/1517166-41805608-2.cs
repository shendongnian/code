                ...
                if (dt.Month == 2 && dt.Day == 29)
                {
                    switch (leapYearIssueAction)
                    {
                        case LeapYearIssue.ThrowException:
                            throw new ArgumentOutOfRangeException("Passed dateTime was in leapyear but target year not", aoorEx);
                        ...
                        default:
                            throw new ArgumentOutOfRangeException("Passed dateTime was in leapyear but target year not. leapYearIssueAction was unknown: " + leapYearIssueAction.ToString(), aoorEx);
                    }
                }
                else
                {
