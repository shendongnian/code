                ...
                while (!reader.EOF)
                {
                    ...
                    if (!reader.EOF)
                    {
                        ...
                        newNetwork.subnetworks = XNetwork.Elements().Where(x => x.Name.LocalName == "SubNetwork").Select(x => new SubNetwork() {
                            ...
                            eNBId = (int)x.Descendants().Where(y => (y.Name.LocalName == "VsDataContainer") && (y.Descendants().Select(z => z.Name.LocalName == "vsDataENodeBFunction").Any())).Select(y => (int)y.Attribute("id")).FirstOrDefault(),
