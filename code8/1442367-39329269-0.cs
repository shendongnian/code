    ...
    public class MultiIntervalTimer: IDisposable
    {
        ...
        {
            IEnumerable<KeyValuePair<int,MultiIntervalTimerCallbackInfo>> callbacksToInvoke = null;
            lock (syncRoot)
            {
                elapsedMiliseconds += interval;
                callbacksToInvoke = callbacks.Where(c => elapsedMiliseconds % c.Key == 0).ToArray();
            }
