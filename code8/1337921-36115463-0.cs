    ...
    {
        int previousWeekCount = 5;
		List<DateTime> listPreviousDates = new List<DateTime>();
		for (int i = 0; i < (previousWeekCount * 7); ++i) //Get previous 5 weeks
		{
			listPreviousDates.Add(DateTime.Today.AddDays(-i));
		}
        ViewData["DateList"] = new SelectList(listPreviousDates);		
        return View();
    }
    [HttpPost]
