    public class ExtendedComparison<T>
    {
        private readonly IKeywordMapper<T> _mapper;
        public ExtendedComparison(IKeywordMapper<T> mapper)
        {
            _mapper = mapper;
        }
        public IOrderedEnumerable<T> BuildQuery(IEnumerable<T> data, params string[] arguments)
        {
            ...
            {
                Func<T, object> lambda = _mapper.Map(sorting[i].Keyword);
                if (i == 0)
