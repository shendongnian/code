    public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
    {
            ...
            if (thing == null)
                throw new ArgumentException($"Writing Json failed because value was not a 'Thing' of type, {typeof(IThing).FullName}");
            JObject jsonThing;
            //If your solution is multithreaded, and is using a shared serializer (which you probably are), you should lock the serializer so that it doesn't accidentally use the "CustomObjectResolver"
            lock (serializer)
            {
