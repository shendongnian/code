        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            ProduceTree(parent, 1);
        }
        private void ProduceTree(Entity parent, int level)
        {
            Debug.WriteLine(new string('-', level) + parent.Name);
            foreach (var child in _entities.FindAll((x) => x.ParentId == parent.Id))
            {
                ProduceTree(child, level + 1);
            }
        }
