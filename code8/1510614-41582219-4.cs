    public class EmailService {
        ...
        public UserData SendEmail(UserData user, string confirmationEmailUrl) {
            try
            {
                ...
                m.Body = string.Format("<img src=\"@@IMAGE@@\" alt=\"\"><BR/><BR/>Hi " + user.FirstName + "," + "<BR/><BR/>Your account has been successfully created with the Company. Please click on the link below to access your account.<BR/><BR/>" + "Username - " + user.UserName + "<BR/>" + "Password - " + user.Password + "<BR/><BR/>" + "<a href=\"{1}\" title=\"User Email Confirm\">Please click here to Activate your account</a>", user.UserName, confirmationEmailUrl + string.Format("<BR/><BR/>Regards,<BR/>The Human Resource Department <BR/>");
