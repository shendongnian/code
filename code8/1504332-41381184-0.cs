        ...
        class Program
        {
            static void Main(string[] args)
            {
                ...
                var lines = matches.Cast<Match>()
                    ...
                    .GroupBy(x => x.name)
                    .Select(x => x.Select((y, i) => i == 0 ? new { index = y.index, s = string.Format("DECLARE @{0} = {1}", x.Key, y.value) } :
                        new { index = y.index, s = string.Format("@{0} = {1}", x.Key, y.value) }))
                    .SelectMany(x => x).OrderBy(x => x.index).Select(x => x.s).ToArray();
