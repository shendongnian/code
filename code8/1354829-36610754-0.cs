    ...
    namespace Cursors
    {
        ...
        /// </summary>
        /// <param name="AssemblyName">The name of the assembly to obtain the resource from.</param>
        /// <param name="ResourcePath">The path to the resource in format {Folder1/Folder2/Resource.Extension}.</param>
        /// <param name="HotX"></param>
        /// <param name="HotY"></param>
        public SmartCursor(string AssemblyName, string ResourcePath, int HotX, int HotY)
        {
            Bitmap Bitmap = new System.Windows.Media.Imaging.BitmapImage(new Uri("pack://application:,,,/" + AssemblyName + ";component/" + ResourcePath, UriKind.Absolute)).ToBitmap();
            SetFromBitmap(Bitmap, HotX, HotY);
        }
            public void Dispose()
            {
