    private static Dictionary<Type, Dictionary<string, Action<object, object>>> _typeMapper;
    public static void Set(object obj, string fieldName, object newValue)
    {
        ...
        if (_typeMapper.TryGetValue(type, out fieldMapper))
        {
            ...
            if (!fieldMapper.TryGetValue(fieldName, out action))
            {
                ...
                action = buildSetter(type.GetField(fieldName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic));
