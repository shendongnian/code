    ...
    {
        var initial = links.GroupBy(x => new { x.Table, x.RefColumn })
            .Select(x => new Dependency {
                TableName = x.Key.Table,
                ColumnName = x.Key.RefColumn,
                Dependencies = x.Select(y => new Dependency { TableName = y.RefTable, ColumnName = y.Column }).ToList() })
            .ToList();
