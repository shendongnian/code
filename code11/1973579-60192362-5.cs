...
----------
**Solution 3 - Introduce a new custom attribute**
csharp
void Main()
{
	string json = @"{
...
public abstract class LegacyAutoMap
{
	...
	private void OnDeserialized(StreamingContext context)
	{
		...
		var mappableProps = this.GetType()
			.GetProperties()
			.Where(p => Attribute.IsDefined(p, typeof(AlternateJSONName)))
			.Select(p =>
			{
				var attrs = p.GetCustomAttributes(typeof(AlternateJSONName)).Cast<AlternateJSONName>();
...
public class AlternateJSONName : Attribute
{
	public string JSONKey { get; }
	public AlternateJSONName(string keyName)
	{
