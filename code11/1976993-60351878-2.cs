...
private void ToTreeView(TreeView tv, DataTable dt1, DataTable dt2, DataTable dt3)
{
    ...
    dt1.Rows.Cast<DataRow>()
        ...
        .ForEach(x =>
        {
            var p = tv.Nodes.Add("job", x.Field<int>("suffix").ToString());
            dt2.Rows.Cast<DataRow>()
            .Where(y => x.Field<int>("suffix") == y.Field<int>("suffix"))
            .ToList()
            .ForEach(y =>
            {
                var c = p.Nodes.Add("oper", $"Oper: {y.Field<int>("operNum").ToString()}");
                dt3.Rows.Cast<DataRow>()
                .Where(z => z.Field<int>("suffix") == y.Field<int>("suffix") &&
                z.Field<int>("operNum") == y.Field<int>("operNum"))
                .ToList()
                .ForEach(n => c.Nodes.Add("seq", $"Seq: {n.Field<int>("seq")}"));
            });
        });
    tv.EndUpdate();
}
Please consider this as a workaround and consider what have been mentioned in the comments. Check out the related posts below.
> **Related**
>
