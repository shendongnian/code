    ...
    using (NpgsqlConnection conn = new NpgsqlConnection(conStr))
    {
        ...
        {
            cmd.Parameters.Add(new NpgsqlParameter("@Id", idReadFromCSV));
            using (NpgsqlDataReader r = cmd.ExecuteReader())
            {
                using (TextWriter tw = new StreamWriter("newCSVFile.csv", false, Encoding.Default))
                {
                    if (r.Read())
                    {
