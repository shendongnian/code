    ...
    public class MoqExpressionTests {
        ...
        public void Should_Build_Moq_Expression() {
            ...
            // It.IsAny<string>()
            var isAnyString = typeof(It).GetMethod("IsAny").MakeGenericMethod(typeof(string));
            var arg = Expression.Call(isAnyString);
            ...
            Expression<Func<IBlah, string>> lambdaExpression = Expression.Lambda<Func<IBlah, string>>(body, parameter);
            var expected = "mocked!";
            m.Setup(lambdaExpression).Returns(expected);
