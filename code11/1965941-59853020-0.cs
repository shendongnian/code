    public static Expression<Func<Movement, bool>> CompareToStr(string searchString) 
    {
    	...
    	foreach (var component in typeof(Movement).GetProperties())
    	{
    		...
    		var toStrCall = Expression.Call(
    				...
    				arg,
    				// link to 'ToString', 
    				// needed to be altered, as if there are nullable types with `null` values, this would cause NRE at runtime
    		...
    		var containsCall = Expression.Call(
    				...
    				Expression.Constant(testString.ToLower())); // since arguments of expression tree should be the expressions
    	...
    	return Expression.Lambda<Func<Movement, bool>>(res);
