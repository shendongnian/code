...
Since you have a `List` reference inside `WareHouseLocation` you have to properly clone this as well by implementing `ICloneable` for `I3DStorageObject` as well, because `MemberwiseClone` copy the reference only, not the referred object
cs
...
You can also look though [MSDN][1] for details and examples of `MemberwiseClone` method and deep/shallow copy of objects
