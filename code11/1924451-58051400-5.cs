    public string Password
    {
      get
      {
        lock ( locker )
        {
          ...
          return Encoding.Default.GetString(StringUtility.Decrypt(buf, _SecureKey.ToString()));
        }
      }
      set
      {
        lock ( locker )
        {
          ...
          {
            var buf = StringUtility.Encrypt(Encoding.Default.GetBytes(value), _SecureKey.ToString());
    ...
    private void CreateSecureKey()
    {
      ...
      foreach ( char c in Convert.ToBase64String(StringUtility.CreateCryptoKey(64)) )
