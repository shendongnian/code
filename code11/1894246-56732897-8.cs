    class MainViewModel
    {
        ...
        {
          // The ComboBox's items source that holds the page view model display names
          this.PageNames= new ObservableCollection<string>() {"First Page", "Second Page"};
          // The Dictionary to get the page view model that maps to a page display name
          this.PageViewModelNameMap = new Dictionary<string, string>()
          {
             {"First Page", nameof(FirstViewModel)},
             {"Second Page", nameof(SecondViewModel)}
          };
          // The Dictionary that stores all page view models that can be retrieved by the view model name
        ...
        // You can use this method as execute handler for your NavigateToPage command
