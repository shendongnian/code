    ...
    var Dt1JoinDt2 = from dr1 in dt1.AsEnumerable()
                      ...
                      select dtJoinedTable.LoadDataRow(
                       (from dc1 in dt1.Columns.Cast<DataColumn>() select new[] {dc1.ColumnName.ToString()} ).ToArray() // cast to DataColumn, use joinDt1AndDt2
                      .Union((from dc2 in dt2.Columns.Cast<DataColumn> select new[] {dc2.ColumnName.ToString()}).ToArray())
                      )
