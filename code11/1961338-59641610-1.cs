...
Although note that assigning *at the start* (where `setValue` is assigned) would also be legitimate.
As it happens, you can *sort of* make this work by using
 c#
[ProtoMember(2, OverwriteList = true)]
but... it works for the wrong reasons IMO, as that could also be implemented with the same pseudo-code as above, but just adding a `.Clear()`, which wouldn't change the output.
