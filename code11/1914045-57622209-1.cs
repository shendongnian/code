public class Program
{
    ...
    {
        const string targetMethodName = "TestMethod";
        var documentEditor = await DocumentEditor.CreateAsync(document);
        var syntaxRoot = await document.GetSyntaxRootAsync();
        var comments = syntaxRoot.DescendantTrivia().Where(t => t.IsKind(SyntaxKind.MultiLineCommentTrivia) || t.IsKind(SyntaxKind.SingleLineCommentTrivia)).ToList();
        // Identify comments which are used to target candidate code to be refactored
        // FIXME we need to cater for any number of blocks
        ...
        var endCommentNode = syntaxRoot.DescendantNodes().First(n => n.SpanStart > nodes.Last().Span.End && n is StatementSyntax);
        documentEditor.ReplaceNode(endCommentNode, endCommentNode.WithoutLeadingTrivia());
