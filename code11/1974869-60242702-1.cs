private string FileR ( string name )
{
    ...
    {
        // I dont believe you would get an error here because if content.length > 0, 
        // that would mean there is at least 1 line that can be read. Since its a string 
        // array, you should always check, string.IsNullOrEmpty(content[i]) which would 
        // cover the case its a space.
