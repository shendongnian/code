...
There are libraries that can help solve this sort of problem. I've used [Bogus](https://github.com/bchavez/Bogus) in a few different projects. I think [AutoFixture](https://github.com/AutoFixture/AutoFixture) is a popular option, but I haven't used it myself. A simple builder is easy to implement, so I recommend starting with a home-brew implementation, and adding a 3rd-party library only if the home-brew implementation becomes too tedious or complicated to maintain. Because the builder is an abstraction itself, it will be easy to replace its implementation with one based on a library if/when the time comes.
