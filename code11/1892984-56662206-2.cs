...
You set `j = 0` when `j == 5` just after `j++`, meaning you set `j` to 0 after checking index 4. While I do not want to encourage such unorthodox styles you could fix it by starting with `j = -1` and putting the `j++` before the use of `j` as an index and then setting `j = -1` again when `j == 5`. Again, this approach makes your code unreadable, please use one of the other solutions.
using System;
