...
### Detailed answer
Blazor works with a virtual dom, the framework tracks changes and send only the changes when main thread is not blocked. This is how I allow Blazor to flush changes to UI:
1. Use `async` functions.
2. Make changes on virtual dom.
3. Don't block main thread for your long process, use an async Task.
