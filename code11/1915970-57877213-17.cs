    ...
    {   
        ...
        for (int i = 0; i < Battlemap.Instance.tiles.Length; i++)
        {
            ...
            int texZPos = Mathf.RoundToInt(Mathf.Lerp(0f, Battlemap.Instance.GridMaxZ - 1.0f, yT));
            colorTex.SetPixel(texXPos, texZPos, selectionColors[(int)Battlemap.Instance.tiles[i].selectionMode]);
        }
        colorTex.Apply();
