...
Note that this isn't an ideal solution, because it uses Reflection every time you want to modify the list. Reflection is generally very slow and inefficient, you might want to find a way to cache the List that is going to be edited. For example, you could do the Reflection when the MonoBehavior is initialized, and then cache the `listToEdit` in a private field, to be accessed/modified later.
...
public class ModifierMBEditor : Editor
{
    ...
    public override void OnInspectorGUI() {
        ...
        modifier.listToEdit = typeof(objectContainingLists).GetField(
