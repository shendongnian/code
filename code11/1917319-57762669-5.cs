    static void Main(string[] args)
    {
        ...
        using (OleDbConnection con = new OleDbConnection(excelCS))
        {
            ...
            using (OleDbCommand excelSource = new OleDbCommand("select ID, Mail_ID, additional from [Sheet1$]", con))
            {
                con.Open();
    ...
    private static void BindGridView()
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("select ID, Mail_ID, additional from tbl_data", con))
            {
                con.Open();
