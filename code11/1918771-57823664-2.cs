    ...
    class Program
    {
        ...
        {
            var list = MakeList();
            var sumExpression1 = (Expression<Func<Data, Int32>>)(d => d.Value1); // Hardcoded
            var sumExpression2 = GeneratePropertyExpression<Data>("Value2"); // Generate property expression
            var sumExpression3 = GenerateSumExpression<DataContainer, Data>("Value3"); // Generate Sum expression
            var selected = list.Select(c => new
            {
                Sum1 = c.Data.Sum(sumExpression1),
                Sum2 = c.Data.Sum(sumExpression2),
                Sum3 = sumExpression3.Compile()(c) // will not need the compile() if you generate the entire select expression. 
            }); ;
