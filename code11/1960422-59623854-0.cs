    ...
    public void CreateLabels(int cols = 3, int rows = 14, bool justReset=false)
    {
    ...
    private void UpdateLabels(List<Dog> dogs, List<Chicken> chickens)
    {
        for (int i = 0; i < dogs; i++)
        {
            if (!dashboardLabels[i, 0].Visible) dashboardLabels[i, 0].Visible = true;
            if (dogs[i].Initialised) BindLabel(dashboardLabels[i, 0], dogs[i],"Name");
        }
    }
