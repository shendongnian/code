...
Note, that I don't know of a good way to pass ViewData information to a Razor page from within the IActionFilter class, so when a user is not authorized, instead of setting `ViewData["Message"] = "Not Authorized"` and calling the "MyAppMessage" view, a static "NotAuthorized" view is called from the `OnActionExecuting` method.
