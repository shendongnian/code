    public void SetColorizationCollectionForShader()
    {
        ...
        Color[] selectionColors = new Color[4] {Color.black, Color.red, Color.green, 
                Color.blue};
        ...
        for(int i = 0; i < coloredTilesArray.Length; i++)
        {
            ...
            float yT = Mathf.InverseLerp(backMostTileZ, forwardMostTileZ, 
                    coloredTilesArray[i].z);
            int texZPos = Mathf.RoundToInt(Mathf.Lerp(0f, tileCountHeight-1.0f, yT));
 
            colorTex.SetPixel(texXPos, texZPos, 
                    selectionColors[coloredTilesArray[i].selectionMode]);
        }
