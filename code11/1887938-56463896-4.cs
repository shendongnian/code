...
class Factories<TKey, TOption>
{
	public Func<TKey, TOption, BaseParser> BaseParserFactory { get; }
	public Func<TKey, TOption, IBulkImport> BulkImportFactory { get; }
	public Func<TKey, TOption, SomethingElse> SomethingElseFactory { get; }
	public Factories(
		Func<TKey, TOption, BaseParser> baseParserFactory,
		Func<TKey, TOption, IBulkImport> bulkImportFactory,
		Func<TKey, TOption, SomethingElse> somethingElseFactory)
	{
...
void UseFactory()
{
	var mappedFactories = new Dictionary<string, Factories<string, ILogger>>
	{
		["A"] = new Factories<string, ILogger>(
		...
		["B"] = new Factories<string, ILogger>(
...
The `Factories` class is what enforces that when there's a `BaseParser` for "A" then there's also a `IBulkImport` and a `SomethingElse`.
