...
`
By having additional struct to be a member of the original struct, you are able to specify the length of both dimensions by setting `SizeConst` in the original struct to the first dimension and setting it to the second direction in the new struct. Making the field private and creating properties for them is merely for convenience, so you don't have to assign the array yourself when creating the struct.  
...
`
And one thing when trying to serialize/deserialize structs like this, be aware of the struct alignment as it can mess with the struct size
