...
in order to get parent and filter out non-matching items, you need a projection stage with `$elemMatch` like so:
java
db.Parent.find({
    "items": {
        "$elemMatch": {
...
var projection = Builders<Parent>.Projection
                                 .Include(p => p.ID)
                                 .ElemMatch(p => p.items, i => i.details.zone.name == "second zone");
var roots = collection.Find(p => p.items.Any(i => i.details.zone.name == "second zone"))
                      .Project(projection)
                      .ToList();
