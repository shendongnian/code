...
You might want to keep in mind that my `Or` is using `OrElse` internally so the second call won't be made if the first returns true. The same goes for `And` with `AndAlso`.  
