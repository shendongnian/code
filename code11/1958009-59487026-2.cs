...
Since [`string`](https://docs.microsoft.com/en-us/dotnet/api/system.string?view=netframework-4.8#remarks) class  is immutable in C#, it's better to use string interpolation or [`string.Format()`](https://docs.microsoft.com/en-us/dotnet/api/system.string.format?view=netframework-4.8) method rather then concatenate them (to avoid an additional copies of strings).
