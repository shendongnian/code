    ...
    var results = await conn.QueryAsync<Person,Address,Note,Tuple<Person,Address,Note>>(cmdTxt, 
       (p,a,n)=>Tuple.Create((Person)p, (Address)a, (Note)n),
       new { personID = 1 });
    
    if(results.Any()) {
        var person = results.First().Item1;   //the person
        var addresses = results.Select(n=>n.Item2); //the person's addresses
        var notes = results.Select(n=>n.Item3);  //all notes for all addresses
        if(addresses.Any()) {
