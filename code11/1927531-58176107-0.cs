...
csharp
myBigGameObject.SetEnable(true/*or false*/);
...
I will highly suggest you to do the *first* or *third* option, unless you h.
The thing is, if you want to go back to the main menu, it shouldn't be very expansive (loading time, memory usage, ...) so you can disable GameObject from the `main scene` (third point) to restore them back quickly when you leave the `main menu` without reloading the entire scene. This will lead you to serialization and deserialization.
