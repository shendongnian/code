    ...
    namespace ConsoleApplication116
    {
        class Program
        {
            static void Main(string[] args)
            {
                ...
                var concat = jobs.Select(x => new LevelDateQuantity() { Date = x.Date, Level = x.Level, Quantity = x.Quantity})
                    .Concat( solutions.Select(x => new LevelDateQuantity() { Date = x.Date, Level = x.Level, Quantity = x.Quantity})).ToList();
                var results = concat.OrderBy(x => x.Level).ThenBy(x => x.Date)
                    .GroupBy(x => new { level = x.Level, date = new DateTime(x.Date.Year, x.Date.Month,1)})
                    .Select(x => new { level = x.Key.level, date = x.Key.date, quantity = x.Sum(y => y.Quantity)})
                    .ToList();
            }
