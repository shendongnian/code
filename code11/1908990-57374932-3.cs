    ...
    private void button2_Click(object sender, EventArgs e)
    {
        ...
        p.Location = new Point(10, DateTime.Now.Seconds * (this.Height / 60)); //"random" Y so they don't pile up
    ...
    {
        for(int i = _panels.Count - 1; i >= 0; i--){ //use a backwards int indexed loop because we are potentially removing items from the list. Working backwards is the easiest way to not have to fiddle the index upon removal
          var p = _panels[i];                        //temp reference to a panel in the list, not related to 'var p' in the button click
          p.Location = p.Location.Offset(25, 0);                        //quick way to move it
          if(p.Location.X > this.Width)              //panel that is off screen?
