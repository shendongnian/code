        ...
        class Program
        {
            ...
            static void Main(string[] args)
            {
                ...
                foreach (XElement scheme in doc.Descendants("scheme"))
                {
                    ...
                    {
                        t999 = employer.Elements("data").Where(x => (string)x.Element("serial-no") == "T999").FirstOrDefault();
                        int maxSerialNumber = employer.Descendants("serial-no").Where(x => (string)x != "T999").Max(x => (int)x);
                        string serialNumberStr = (t999 == null) ? "T999" : (maxSerialNumber + 1).ToString();
                        newData = NewData(
                             serialNumberStr,
                             "",  //pin
