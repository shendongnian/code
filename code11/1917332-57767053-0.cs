    ...
    internal static bool CanReadExt(PropertyInfo pi)
    {
      ...
      var basePi = baseSetter.DeclaringType.GetProperties().SingleOrDefault(x => x.SetMethod == baseSetter)
        ?? throw new Exception("Set accessor was overridden but could not find propery info for base property.");
