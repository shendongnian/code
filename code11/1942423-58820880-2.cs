        private void ClickRandomLinks(string linkText = null)
        {
            ...
            if (!string.IsNullOrEmpty(linkText))
            {
                ...
                var randomElements = Enumerable.Range(0, linkElements.Count).OrderBy(x => Guid.NewGuid()).ToList();
                randomElements.ForEach(x => linkElements.ElementAt(x).Click());
            }
            else
            {
                //click all links randomnly
                var randomElements = Enumerable.Range(0, links.Count).OrderBy(x => Guid.NewGuid()).ToList();
