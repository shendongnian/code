    ...
    ProcessData(items,
        new DataSelector<Tuple<T1,T2,T3>>("head1", d => d.Prop1),
        new DataSelector<Tuple<T1,T2,T3>>("head2", d => d.Prop2));
                                   ...
                                   params ColumnDef<T>[] defs) 
    {
        foreach (var item in stuff) {
             // Use all defs like...
             Console.WriteLine(defs[0].Header + ": " + defs[0].Selector(item).ToString());
        }
    }
