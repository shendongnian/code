    ...
    public struct Index : IEnumerable<int>, ICloneable
    {
        ...
        /// <summary>
        /// Lowest Index in Range
        /// </summary>
        public int Min { get { return index_list.Length>0?index_list[0]:-1; } }
        /// <summary>
        /// Highest Index in Range
        /// </summary>
        public int Max { get { return index_list.Length>0?index_list[index_list.Length-1]:-1; } }
        /// <summary>
        /// Extracts elements from an array based on the indexes in the range
        /// </summary>
        public T[] Extract<T>(T[] array)
        {
            T[] elements=new T[index_list.Length];
            for(int i=0; i<index_list.Length; i++)
            {
                elements[i]=array[index_list[i]];
            }
            return elements;
        }
        /// <summary>
        /// Injects elements int an array based on the indexes in the range
        /// </summary>
        public void Inject<T>(T[] array, T[] elements)
        {
            for(int i=0; i<index_list.Length; i++)
            {
                array[index_list[i]]=elements[i];
            }
        }
        #region IEnumerable
        public IEnumerator<int> GetEnumerator()
        {
            for(int i=0; i<index_list.Length; i++) { yield return index_list[i]; }
        }
