        ...
        if (ModelState.IsValid) {
            ...
            if (login != null) {
                Task.Run(() => Logger.LogAsync(Action.UserSignIn, Level.Log, string.Format("User signed in as '{0}' -> '{1}'", model.Username, login)));
                return Request.IsAjaxRequest() ? Content("__close__") : ClientCabinet();
            }
           Task.Run(() => Logger.LogAsync(Action.UserSignIn, Level.Mandatory, Event.Error, string.Format("User failed to sign in as '{0}': invalid username or password", model.Username)));
