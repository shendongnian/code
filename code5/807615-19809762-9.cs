	public class Month : DependencyObject
	{
		...
		public static readonly DependencyProperty SelectedDayProperty =
			...
			new UIPropertyMetadata(null, (d, e) =>
			{
				...
				{
					viewModel.SelectedMonth = viewModel.Months.FirstOrDefault(x => x.Number == ((Month)d).Number + 1);
					if (viewModel.SelectedMonth != null)
						viewModel.SelectedMonth.SelectedDay = viewModel.SelectedMonth.Days[1];//first real day of month
				}
				else if (((Day)e.NewValue).IsFirstDayOfMonth)
				{
					...
					if (viewModel.SelectedMonth != null)
						viewModel.SelectedMonth.SelectedDay = 
							viewModel.SelectedMonth.Days[viewModel.SelectedMonth.Days.Count - 2];//last real day of month
				}
			}));
	}
