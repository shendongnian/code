    ...
    { class Program
    {
        ...
        private static void Fredou(string input, string replace, string[] replaceBy)
        {
            ...
            Parallel.For(0, replaceBy.Length, i =>
            {
                for (int j = 0; j < inputLength; ++j)
                {
                    if ((j + 1) < inputLength && input[j] == replace[0] && input[j + 1] == replace[1])
                    {
