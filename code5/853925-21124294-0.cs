    ...
    public static async Task<string> GetLocalImageAsync(string internetUri)
    {
        ...
        if (localFolder == null)
        {
            return PocketArticle.OfflinePhotoImgPath;
        }
        // Default to offline photo
        string src = PocketArticle.OfflinePhotoImgPath;
        try
        {
            using (var response = await HttpWebRequest.CreateHttp(internetUri).GetResponseAsync())
            {
                using (var stream = response.GetResponseStream())
                {
                    ...
                    var imageFolder = await localFolder.CreateFolderAsync(CachedImagesFolderEndFolderPath, CreationCollisionOption.OpenIfExists);
                    var file = await imageFolder.CreateFileAsync(fileName, CreationCollisionOption.ReplaceExisting);
                    // Copy bytes from stream to local file without changing any file information
                    using (var filestream = await file.OpenStreamForWriteAsync())
                    {
                        await stream.CopyToAsync(filestream);
                        // Send back the local path to the image (including 'ms-appdata:///local/cache/')
                        return string.Format(CachedImagesFolderFullPath + "{0}", fileName);
                    }
                }
            }
        }
        ...
        {
            // Is implicitly handled with the setting of the initilized value of src
        }
    ...
    public static async Task<string> ResizeAndCompressLocalImage(string imgSrc)
    {
        ...
        string sourcepathShort = imgSrc.Replace(CachedImagesFolderFullPath, string.Empty);
        // Get the cached images folder
        var folder = await ApplicationData.Current.LocalFolder.GetFolderAsync(CachedImagesFolderEndFolderPath);
        // Get a new random name (e.g. '555jkdhr5.jpg')
        var targetPath = string.Format("{0}.jpg", Path.GetFileNameWithoutExtension(Path.GetRandomFileName()));
        ...
        {
            using (var destFileStream = await targetFile.OpenAsync(FileAccessMode.ReadWrite))
            {
                ...
                double proportionWidth = (double)decoder.PixelWidth / LayoutDimensions.MaximumColumnWidth;
                double proportionImage = decoder.PixelHeight / (double)decoder.PixelWidth;
                // Get the new sizes of the image whether it is the same or should be resized
                var newWidth = proportionWidth > 1 ? (uint)(LayoutDimensions.MaximumColumnWidth) : decoder.PixelWidth;
                var newHeight = proportionWidth > 1 ? (uint)(LayoutDimensions.MaximumColumnWidth * proportionImage) : decoder.PixelHeight;
                // Prepare set of properties for the bitmap
                BitmapPropertySet propertySet = new BitmapPropertySet();
                    
                // Set ImageQuality
                BitmapTypedValue qualityValue = new BitmapTypedValue(0.75, PropertyType.Single);
                propertySet.Add("ImageQuality", qualityValue);
                //BitmapEncoder enc = await BitmapEncoder.CreateForTranscodingAsync(destFileStream, decoder);
                BitmapEncoder enc = await BitmapEncoder.CreateAsync(BitmapEncoder.JpegEncoderId, destFileStream, propertySet);
        ...
        // Return the new path including "ms-appdata:///local/cache/"
        return string.Format(CachedImagesFolderFullPath + "{0}", targetPath);
    }
