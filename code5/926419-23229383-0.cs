    private static List<int> AllPossibleCombinations(IList<int> alphabet)
    {
        ...
        MakeCompbination(combinations, alphabet.Count, new int[0]); // Start recursion
    ...
    private static void MakeCompbination(List<int[]> output, int length, int[] usedIndices)
    {
        ...
        for (int i = 0; i < length; i++)
            if (Array.IndexOf(usedIndices, i) == -1)    // If the index wasn't used earlier
            {
                ...
                if (length + 1 != indices.Length)
                    MakeCompbination(output, length, indices);  // Recursion
            }
    }
