    ...
    namespace Console_21628490
    {
        ...
        class Program
        {
            static async Task DoWorkAsync()
            {
                ...
                try
                {
                    ...
                    Console.ReadLine();
                    worker.PauseExecutingTasks();
    
                    // resume upon Enter
                    Console.WriteLine("\nPress Enter to resume...");
                    Console.ReadLine();
                    worker.ResumeExecutingTasks();
                    ...
                    worker.EndExecution(); // or worker.Cancel() to throw
        ...
        public class WorkerWithTaskScheduler<TResult> : TaskScheduler, IDisposable
        {
            ...
            // make ExecutePendingTasks block
            public void PauseExecutingTasks()
            {
                _waitForTasks = true;
            }
    
            // make ExecutePendingTasks unblock
            public void ResumeExecutingTasks()
            {
                Run(() => _waitForTasks = false, this.Token);
            }
