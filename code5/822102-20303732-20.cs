        private unsafe static void FredouImplementation(string input, int inputLength, string replace, string[] replaceBy)
        {
            ...
            Parallel.For(0, replaceBy.Length, l =>
                Process(input, inputLength, replace, replaceBy[l], idx, len)
            );
        }
        private unsafe static void Process(string input, int len, string replace, string replaceBy, int[] idx, int idxLen)
        {
            var output = new char[len];
            int l;
            fixed (char* o = output, i = input, r = replace)
            {
