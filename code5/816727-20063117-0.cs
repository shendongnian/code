    ...
    namespace LinqToObjects
    {
    	...
    	class Program
    	{
    		static void Main(string[] args)
    		{
    			...
    			var query = lst.GroupBy(obj => new { obj.Qid, obj.Uid}, ( key,group) => new {Key1= key.Qid, Key2 = key.Uid, Count = group.Average(o=>o.BS)});
    
    			Console.WriteLine("Qid\tUid\tWeightedAvg");
    			foreach(var item in query)
    			{
    				Console.WriteLine(string.Format("{0}\t{1}\t{2}",item.Key1,item.Key2,item.Count));
    			}
    		}
    	}
    }
