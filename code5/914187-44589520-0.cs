            ...
            foreach (var ua in adapterProperties.UnicastAddresses)
            {
				...
				//RECEIVE BROADCAST IN THE ADAPTER
                    ...
                    do
                    {
                        try
                        {
                            bcSocket.Receive(so.bufferAnswer);
                            DevicesList.Add(GetMyDevice(so.bufferAnswer)); //Corresponding functions to get the devices information. Depends on the application.
                        }
                        catch { break; }
