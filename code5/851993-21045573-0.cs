    ...
    namespace WhatWasTheQuestion {
        class Program {
            ...
            static bool generateAndCheckCandidate(int X) {
                var prefixCandidates = prefix.OrderBy(x => random.Next(0, prefix.Length)).Take(random.Next(3, 6)).ToList();
                var suffixCandidates = suffix.OrderBy(x => random.Next(0, suffix.Length)).Take(random.Next(3, 6)).ToList();
