    ...
    namespace My.ActionFilters
    {
        ...
        public class ThrottleFilterAttribute : ActionFilterAttribute
        {
            ...
            var allowExecute = memCache.TryGetValue(key, out int counter);
            memCache.Set(key, ++counter, new MemoryCacheEntryOptions() { SlidingExpiration = TimeSpan.FromMilliseconds(Milliseconds) });
