    ...
    public override void OnAuthentication(HttpAuthenticationContext context)
    {
        ...
        //First of all we are going to check that the request have the required Authorization header. If not set the Error
        ...
        if (authHeader == null || authHeader.Scheme != "Bearer")
        {
            context.ErrorResult = context.ErrorResult = new AuthenticationFailureResult("unauthorized", context.Request,
                new { Error = new { Code = 401, Message = "Request require suthorization" } });
        }
