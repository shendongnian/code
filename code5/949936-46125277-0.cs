        ...
        public class FastTextBox : TextBox
        {
            ...
            private string GetNewText(string oldText, string input, int offset, int length)
            {
                if (input.Length == 1 ? input.ToCharArray()[0] == (int)Keys.Back : false)
                    return $"{oldText.Substring(0, offset - (length == 0 ? 1 : 0))}{oldText.Substring(offset + length)}";
                else
            ...
            protected override void WndProc(ref Message m)
            {
                ...
                {
                    case 0x102:
                        text = ((char)m.WParam.ToInt32()).ToString();
                        goto case 0x302;
                    case 0x302:
                    case 0xC:
                        string input = string.IsNullOrEmpty(text) ? Marshal.PtrToStringUni(m.LParam) : text, newText = string.Empty;
