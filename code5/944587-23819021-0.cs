    ...
    namespace AsyncWebBrowserScrapper
    {
            ...
            static async Task ScrapSitesAsync(string[] urls, CancellationToken token)
            {
                using (var pool = new WebBrowserPool(maxParallel: 2, token: token))
                {
                    foreach (var url in urls)
                    {
                        ...
                        string html = await pool.ScrapSiteAsync(url, (int)TimeSpan.FromSeconds(30).TotalMilliseconds);
        ...
        public class WebBrowserPool : IDisposable
        {
            ...
            SemaphoreSlim _semaphore; // regulate available browsers
            Queue<WebBrowser> _browsers; // the pool of available browsers
            HashSet<Task> _pendingTasks; // keep track of pending tasks for proper cancellation
