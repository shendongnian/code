    var searchStream = Observable.FromEventPattern(
        ...
        .Select(searchTerm =>
        {
            ...
            return foundParties
                // this ToObs gets us into the monad from Task
                .ToObservable()
                // this ToObs gets us into the monad from IEnum<T>
                .SelectMany(x => x.Buffer(500).ToObservable())
                // placement of this is again important to avoid races updating the UI
                .ObserveOn(SynchronizationContext.Current); 
        })
        .Switch() // This cancels a prior result if superceded by a textbox change
