    public class FindTextChangedBehavior : Behavior<TextBox>
    {
        ...
        private async void OnTextChanged(object sender, TextChangedEventArgs args)
        {
            ...
            if (textBox != null)
            {
                try
                {
                    ...
                    IEnumerable<GridProperty> interestingProperties = this.GetInterestingProperties();
                    // This appears to be CPU-bound, so Task.Run is appropriate.
                    ObservableCollection<SearchMarker> tempItems = await Task.Run(
        ...
        private ObservableCollection<SearchMarker> ResolveSearchMarkersAsync(
            IEnumerable<GridProperty> interestingProperties, CancellationToken cancellationToken)
        {
            var tempItems = new ObservableCollection<SearchMarker>();
