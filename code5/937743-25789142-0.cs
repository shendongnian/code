    ...
    using (var query = currentConnection.QueryMultiple(UspDSPGetBehavioralCategoriesTest, filters, nhTransaction, commandType: CommandType.StoredProcedure))
    {
    	var countRows = query.Read<int>().FirstOrDefault();
    	var temp = query.Read<BehavioralCategoryModel>().ToList();
    	return new Result<IEnumerable<BehavioralCategoryModel>>(temp, countRows);
    }
