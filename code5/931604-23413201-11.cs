	protected override void Render(HtmlTextWriter writer)
	{
		...
		using (HtmlTextWriter htmlwriter = new HtmlTextWriter(new StringWriter()))
		{
			...
			/* To support unicode \w, \d and \p{X} variables inside of asp:RegularExpressionValidator controls on client side.
			...
			* Do not use \W, \D and \P{X} with it, instead negate it with ^.
			*/
			html = Regex.Replace(html, @"(?<=\.validationexpression = "".*?)\\\\w(?=.*?(?<!\\)"")", "\" + w + \"");
			html = Regex.Replace(html, @"(?<=\.validationexpression = "".*?)\\\\d(?=.*?(?<!\\)"")", "\" + d + \"");
