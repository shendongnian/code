    public static class MoqSetupExtensions
    {
        ...
        public static Mock<DbSet<T>> createFakeDBSet<T>(this Mock<FakeDbContext> db, List<T> list = null, Func<List<T>, object[], T> find = null) where T : class
        {
            ...
            //Setup all Concrete classes
            var type = typeof(T);
            if (type.IsAbstract)
            {
                var concreteTypes = domainTypes.Where(x => type.IsAssignableFrom(x) && !x.IsAbstract && type != x).ToList();
