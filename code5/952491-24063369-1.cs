    public partial class ButtonGrid : UserControl
        {
            ...
            private void OnClick(object sender, RoutedEventArgs e)
            {
                ...
                {
                    number = Int32.Parse(button.Name.Substring(6, 1));
                }
                else
                {
                    number = Int32.Parse(button.Name.Substring(6, 2));
                }
    
                // Make sure the buttons that are being affected are within bounds
                if ((number - 1) >= 0)
                {
                    buttons[number - 1].IsEnabled = true;
                }
    
                if ((number - rows) >= 0)
                {
                    buttons[number - rows].IsEnabled = true;
                }
    
                if ((number + 1) <= (cols * rows - 1))
                {
                    buttons[number + 1].IsEnabled = true;
                }
    
                if ((number + rows) <= (cols * rows - 1))
                {
                    buttons[number + rows].IsEnabled = true;
                }
            }
