        ...
        class Program
        {
            ...
            public class CompareAttribut : IComparer<Attribut>
            {
                ...
                public int Compare(Attribut x, Attribut y)
                {
                    ...
                    if (outcome == 0)
                    {
                        if (x.ATT_ID == y.ATT_PARENT_ID) outcome = -1; //Parent before child
                        else if (x.ATT_PARENT_ID == y.ATT_ID) outcome = 1; //Child after parent
                        //SortLevel2
                        else outcome = x.ATT_LIBELLE.CompareTo(y.ATT_LIBELLE) * (order ? 1 : -1);
                    }
                    return outcome ;                
                }
            }
