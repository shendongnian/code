    ...
    namespace ConsoleApp1
    {
        class Program
        {
            ...
            static void Main(string[] args)
            {
                ...
                sortList.Sort(new CompareAttribut(sortList, true));
            ...
            public class CompareAttribut : IComparer<Attribut>
            {
                ...
                public int Compare(Attribut x, Attribut y)
                {
                    ...
                    while (i < xParentLib.Length)
                    {
                        if (i < yParentLib.Length && x.ATT_PARENT_ID == y.ATT_ID) return 1;//Child after parent
    
                        int outcome = xParentLib[i].CompareTo(yParentLib[i]) * (order ? 1 : -1);
                        if (outcome == 0)
                        {
                            i++;
                            continue;
