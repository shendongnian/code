    ...
    {
       List<RMA_History> results = getRMAHistory(LikeOrderNummer, From, Amount, skip);
    ...
    private List<RMAHistory> getRMAHistory(string LikeOrderNummer, int From, int Amount, int skip)
    {
       ...
       .Where(a => a.y.Email == EmailID && LikeOrderNummer == "" ? true : a.y.Ordrenummer.StartsWith(LikeOrderNummer.Trim()) || a.y.Fakturnummer.StartsWith(LikeOrderNummer.Trim())).Distinct().Select(t => new RMAHistory
        {
            Status = t.u.Status,
