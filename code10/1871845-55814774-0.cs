public class MyModelBinder : IModelBinder
{
    ...
    //DO NOT transmit a type FullName and use Type.GetType(fullName), this could cause a RCE vulnerability.
...
> NOTE: When doing custom deserialization with a Type, it is important to limit the list of allowed types to be deserialized. If you would accept a type's FullName and use `Type.GetType(fullName)`, this could cause a RCE vulnerability since there are some [types][1] in .NET that execute code when a property is set. 
