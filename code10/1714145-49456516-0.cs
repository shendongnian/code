    ...
    public class NewBehaviourScript : MonoBehaviour {
        ...
        void Start()
        { StartCoroutine(EnemyFadeIn(fadeTime, waitTime)); }
        ...
        IEnumerator EnemyFadeIn(float timeToFade, float timeToWait) {
            ...
            yield return ListenForInput(timeToFade, timeToWait);
        }
    
        IEnumerator EnemyFadeOut(float timeToFade, float timeToWait) {
            ...
            yield return EnemyFadeIn(timeToFade, timeToWait);
        }
    
        // Responsible for reacting to the 'S' key input.
        IEnumerator ListenForInput(float timeToFade, float timeToWait) {
            canRespond = true;
            ...
            while (Time.time < (startTime + timeToWait)) {
                if (hasResponded) {
                    ...
                    hasResponded = false;
                    canRespond = false;
                    yield return EnemyFadeOut(timeToFade, timeToWait);
                }
