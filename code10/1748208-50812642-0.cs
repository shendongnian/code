    ...
    {
        private static int Add<TKey>(Dictionary<TKey, int> dict, TKey key)
        {
            int value;
        ...
        public static IEnumerable<TSource> OrderByPartition<TSource, TKey>(this IEnumerable<TSource> source, Func<TSource, TKey> keySelector)
        {
            ...
            var res = source.Select(x => new { Value = x, Partition = Add(dict, keySelector(x)) }).OrderBy(x => x.Partition).ThenBy(x => keySelector(x.Value));
