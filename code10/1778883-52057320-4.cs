    public class CitiesCollection : IReadOnlyCollection<City>
    {
        ...
        public IEnumerable<City> Find(string find)
        {
            ...
            foreach (var city in _suffixdict.GetDistinct(normalizedFind).Where(v => v.Name.IndexOf(normalizedFind, StringComparison.OrdinalIgnoreCase) >= 0))
                yield return city;
        }
        public IEnumerator<City> GetEnumerator()
        {
            return _suffixdict.GetEnumerator();
        }
    ...
    public class SuffixDict<T> : IReadOnlyCollection<T>
    {
