...
3. Lastly, if you can't determine the type at compile-time, and want to resolve type dynamically, **you could inject an `IServiceProvider` instead of the `IApplicationBuilder`**. In this way, You could resolve any instance as you like :
...
So you could pass an `IServiceProvider` to `GenerateExcelFile(IServiceProvider sp, IEnumerable<string> hashSetCodes, ContestViewModel model)`, and within the `GenerateExcelFile()` method, you could resolve the `AppDbContext` in the following way:
