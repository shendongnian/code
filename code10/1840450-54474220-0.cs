    ...
    public void LoadAsstBundles(int choice)
    {
        ...
        if(!assetInstance)
        {
            assetInstance = (GameObject)Instantiate(asset, ParentTransform);
    
            // Add all child's of Instantiated object to the choices list
        ...
        foreach(var chair in choices)
        {
            chair.SetActive(chair == selected);
            }
        }
    }
