...
You can call `Basket.ProceedToCheckout`, which would change its internal state by setting a `Closed` property to `true`. Then the creation of the Order could go wrong and you would __not__ need to rollback the Basket.
