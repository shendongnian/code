    public async Task Fn(..., CancellationToken cancellationToken)
    {
        ...
        dbContext.SaveChangesAsync(cancellationToken); // save the pending appointment
        ...
        {
            // this means that another appointment was added after our first check.
            // we have to remove the pending appointment, concurrency check failed
            dbContext.Appointments.Remove(appointment);
            dbContext.SaveChangesAsync(cancellationToken); // restore DB
            return BadRequest(); // same response as before
        }
        
        // ok, we can make remove the pending state
        appointment.PendingUntil = null;
           
        dbContext.SaveChangesAsync(cancellationToken); // now consistency is granted
        return Ok();
    }
