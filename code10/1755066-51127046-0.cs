    ...
    public sealed class CatalogueLazyLoad : LocationInterceptionAspect, IInstanceScopedAspect
    {
        ...
        [ImportMember("LoadedCategories", IsRequired = true, Order = ImportMemberOrder.AfterIntroductions)]
        public Property<HashSet<string>> LoadedCategoriesProperty;
        ...
        public override sealed void OnGetValue(LocationInterceptionArgs args)
        {
            ...
            switch (Cache)
            {
                case CacheType.CACHED:
                    bool loaded = this.LoadedCategoriesProperty.Get().Contains(this.Charset);
                    if (!loaded)
                    {
                        GetValue(args, true);
                    }
        ...
        {
            this.LoadedCategoriesProperty.Set(new HashSet<string>());
        }
    }
