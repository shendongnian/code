    // Arrange
    // Let's set up the mock so when Foo is called, it will return 5
    var iOneMock = new Mock<IOne>();
    iOneMock.Setup(x => x.Foo()).Returns(5);
    // Let's set up the mock so when Foo is called with any string, 
    // it will return 1
    ...
    some.Work();
    // Assert
    // Let's verify Foo was called. 
    iOneMock.Verify(x => x.Foo());
    // Let's verify Foo was called with string "One" and was called only once
