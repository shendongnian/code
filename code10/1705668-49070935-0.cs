    public static List<NumericUpDown> lUpDown = new List<NumericUpDown>();      //List of Updowns we will be using
        private void treeView1_AfterExpand(object sender, TreeViewEventArgs e)
        {
            ...
            {
                NumericUpDown newUpDown = new NumericUpDown() { Name = "upDown" + e.Node.Index.ToString() };    //Making a NumericUpDown with a unique name linked to the index of the subnode(Apple juice = 0, Orange Juice = 1, ...)
        ...
        private void treeView1_AfterCollapse(object sender, TreeViewEventArgs e)
        {
            if (e.Node.Level == 0)
            {
                foreach (Control updown in lUpDown)
                {
                    Controls.Remove(updown);
                }
            }
            else
            {
                NumericUpDown UpDownToRemove = lUpDown.Find(x => x.Name == "upDown" + e.Node.Index.ToString());     //Finding by the index which NumericUpDown we'll remove
            ...
            foreach (TreeNode Node in tv.Nodes)     //Going through all nodes in the TreeView(in this case, only Drinks)
            {
                foreach (TreeNode subNode in Node.Nodes)        //Going through all subnodes(childs of Drinks: Apple Juice, Orange Juice, ...)
                {
                    ...
                    if (subNode.IsExpanded)
                    {
                        ...
                        lUpDown.Find(x => x.Name == "upDown" + index.ToString()).Location = upDownLoc;      //Finding the correct UpDown to update location by the subnode index(Apple juice = 0, Orange Juice = 1, ...)
                    }
                }
            }
        }
