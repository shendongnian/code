            ...
            foreach (string path in listBox.Items) // file(paths) are stored in a ListBox
            {
                ...
                {
                    row[i] = value; // inserts value column by column for each parameter
                    i++;
                }
                table.Rows.Add(row); // add the whole row to the DataTable
            }
        dataGridView1.DataSource = table; // bind our DataTable to DataGridView
