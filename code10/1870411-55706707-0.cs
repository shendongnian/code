...
The `OlDaysOfWeek` enum utilises a power-of-2 sequence, which is typically used when combining values as bitwise flags. `DaysOfWeek` has a simple linear sequence reflected as 1-7 (where Monday is the first day of the week) - this is why you can't compare using the backing `int` value. Additionally, `OlDaysOfWeek` considers Sunday to be the first day of the week
