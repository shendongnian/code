            ...
            using (ManagementObjectSearcher search = new ManagementObjectSearcher(_Scope, _Query, _Options))
            {
                foreach (ManagementObject _EventLog in search.Get())
                {
                    LogEvents.Add(new WinLogEvent { 
                               ...
                               TimeGenerated = _EventLog.GetPropertyValue("TimeGenerated").ToString(),
                               TimeLogged = _EventLog.GetPropertyValue("TimeWritten").ToString(),
                               Message = _EventLog.GetPropertyValue("Message").ToString(),
