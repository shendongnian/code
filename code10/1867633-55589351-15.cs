    ...
    class Program
    {
        static void Main()
        {
            ...
            foreach (var zeit in dates)
                Console.WriteLine(zeit.HasValue ? zeit.Value : "<empty>");
            interpolate_dates(dates);
            Console.WriteLine("\nAfter:");
            foreach (var zeit in dates)
                Console.WriteLine(zeit.HasValue ? zeit.Value : "!!ERROR!! - all dates should be interpolated.");
        }
        static public void interpolate_dates(Span<DateTime?> dates)
        {
            if (dates.Length == 0)
                return;
            if (!dates[0].HasValue)
                throw new ArgumentException("First date value cannot be null.");
