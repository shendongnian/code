...
This produces a perfect initial migration (where `migrationBuilder.CreateTable` is used), and generates the expected SQL. This *should* also work on migrations that update existing tables, but do make sure that `defaultValue` is always null.
