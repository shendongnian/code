                ...
                using (var workbook = SpreadsheetDocument.Create(file_path, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    for (int row = 0; row < dgv.RowCount; row++)
                    {
                        ...
                        for (int col = 0; col < dgv.ColumnCount; col++)
                        {
                          ...
                          // Datatype=CellValues.Number. If someone knows answer. Please let me know. However, Data format strangely work fine with Number datatype
                         // Alos important – whatever format you define in creating stylesheets, you have to insert value of same kind in string here – for CellValues !
    // I used cell formating as I needed, for something else just change Worksheet_Style method to your needs
                            ...
                            {
                                cell.DataType = new EnumValue<CellValues>(CellValues.String);
                                cell.CellValue = new CellValue(dgv.Rows[vrstica].Cells[col].Value.ToString());
                                cell.StyleIndex = 0;
                            }
            ...
            {
                WorkbookStylesPart create_style = dokument.WorkbookPart.AddNewPart<WorkbookStylesPart>();
    ...
    //now append column to worksheet, calculations done
            ...
            {
                // This formula calculates width. For better desired outputs try to change 0.5M to sometinh else
