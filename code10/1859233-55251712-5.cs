    ...
    namespace WindowsFormsApp2
    {
        public partial class Form1 : Form
        {
            ...
            private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
            {
                if (hookForm.Handle != GetForegroundWindow())
                {
                    if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN)
                    {
                        ...
                        if (!bShortcutPressed && dicTest.ContainsValue(hookKey)) { hookForm.OnKeyDown(new System.Windows.Forms.KeyEventArgs(hookKey)); bShortcutPressed = true;
    
                            Debug.Print($"{bShortcutPressed}");
                        }
                    }
    
                    if (nCode >= 0 && wParam == (IntPtr)WM_KEYUP) { bShortcutPressed = false; Debug.Print($"{bShortcutPressed}"); }
                }
            ...
            private void Form1_Load(object sender, EventArgs e)
            {
                ...
                dicTest.Add("test", (Keys.Control | Keys.U));
            }
            ...
            {
                Debug.Print($"Button2 was clicked");
            }
        }
    }
