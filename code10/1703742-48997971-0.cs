    ...
    internal static class ContractResolverExtensions
    {
        ...
        static Func<object, object> CreateMethodGetterGeneric<TObject, TValue>(MethodInfo method)
        {
            ...
            return (o) => (TValue)func((TObject)o);
        }
        ...
        static IValueProvider ToValueProvider(MethodInfo method, Type type)
        {
            try
            {
                return new MethodProvider(CreateMethodGetter(method, type));
            }
            catch (Exception ex)
            {
                Debug.WriteLine(ex);
