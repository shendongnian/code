    public class UsersViewModel : BindableBase
    {
        ...
        {
            _usersView = CollectionViewSource.GetDefaultView(HddUsers);
            _usersView.Filter = delegate (object item)
            {
                User user = item as User;
                List<string> columns = new List<string>() { user.NationalIdentificationNumber, user.FullName, user.Platoon };
                bool include = true;
