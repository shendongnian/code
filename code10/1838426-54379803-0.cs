      public void xPix(Bitmap bmp, int n, Color cx, Color nx)
      {
         ...
         for (int y = 0; y < img.Height; y++)
         {
            ...
            {
               cx = Color.FromArgb(BitConverter.ToInt32(bmpBytes, y * img.Stride + x));
               if (x + n <= img.Width - 1) nx = Color.FromArgb(BitConverter.ToInt32(bmpBytes, y * img.Stride + x));
               BitConverter.GetBytes(nx.ToArgb()).CopyTo(bmpBytes, y * img.Stride + x);
               if (x + n <= img.Width - 1) BitConverter.GetBytes(cx.ToArgb()).CopyTo(bmpBytes, y * img.Stride + x + n);
            }
         }
