    public class TaskContainer
        ...
        {
            var manifest = this.GetManifest(Properties.Settings.Default.ResourceId);
            if (manifest.Processor.Count() > 0)
            {
                foreach (var task in manifest.Processors)
                {
        ...
        private void CreateProcessorTask(Processor processor)
        {
            ...
            Task taskInstance = Task.Factory.StartNew(
                () => processor.Start(cancellationTokenSource.Token, this)
            );
