    ...
    {   public  string last_path="";
        ...
        private static void Consumer()
        {
            foreach (var message in messages.GetConsumingEnumerable())
            {
                ...
                int maxchar = 10240;
    			 if (last_path !="")
    				 {
    				int length = new System.IO.FileInfo(last_path).Length;
    				 if (length< maxchar )
    					{
    					  maxchar =maxchar -length;
    					  dividefile(last_path,  maxchar) 
    					} 
    	...
    	public static void dividefile(string path,int  maxchar)
            {
