    ...
    {
        private static IDictionary<string, PropertyInfo[]> PropertyMap = new Dictionary<string, PropertyInfo[]>();
        public static IDictionary<string, T> ToDictionary<T>(this object obj)
        {
            var objType = obj.GetType();
            if (!PropertyMap.TryGetValue(objType.FullName, out var props))
            {
                PropertyMap.Add(objType.FullName, props = objType.GetProperties());
            }
