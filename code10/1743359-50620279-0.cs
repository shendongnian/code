	...
	void SearchAndDisplay(UserManager userManager, string searchByUsername, string searchByGivenName, string searchBySurname)
	{
		...
		if (!string.IsNullOrEmpty(searchByUsername)) searchCriteria = And<User>(searchCriteria, user => searchByUsername.Equals(user?.Username));
		if (!string.IsNullOrEmpty(searchByGivenName)) searchCriteria = And<User>(searchCriteria, user => searchByGivenName.Equals(user?.GivenName));
		if (!string.IsNullOrEmpty(searchBySurname)) searchCriteria = And<User>(searchCriteria, user => searchBySurname.Equals(user?.Surname));
