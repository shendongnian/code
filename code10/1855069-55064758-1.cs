    private void Area_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            foreach (object area in Area.SelectedItems)
            {
                ...
                SqlDataAdapter adpt = new SqlDataAdapter("SELECT PostCode,Town FROM tblAllPostCodes where Town in (" + string.Join(",", Area.SelectedItems.Select(si => "'" + si.ToString() + "'").ToArray()) + ")", sqlConTwo);
                adpt.Fill(dt);
