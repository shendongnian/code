    ...
    private void ReadCompoWithLinq() {
            ...
            List<Compo> compos = doc.Descendants("COMPO").Select(x => new Compo()
            {
                ...
                min = (x.Attribute("missing") != null) ? null : (string)x.Element("min"),
                max = (x.Attribute("missing") != null) ? null : (string)x.Element("max"),
                code_confiance = (string)x.Element("code_confiance"),
                source_code = (x.Attribute("missing") != null) ? null : (string)x.Element("source_code")
            }).ToList();
    }
