    public partial class Form1 : Form
    {
        ...
        private void UpdateDirectionAndDistance()
        {
            ...
            if (pnlBacteria.Top < bacteriaSize)
                validDirections = validDirections.Except(
                    new List<Direction>
                    { Direction.North, Direction.NorthEast, Direction.NorthWest});
            if (pnlBacteria.Right > ClientRectangle.Width - bacteriaSize)
                validDirections = validDirections.Except(
                    new List<Direction>
                    { Direction.East, Direction.NorthEast, Direction.SouthEast });
            if (pnlBacteria.Left < bacteriaSize)
                validDirections = validDirections.Except(
                    new List<Direction>
                    { Direction.West, Direction.NorthWest, Direction.SouthWest});
            if (pnlBacteria.Bottom > ClientRectangle.Height - bacteriaSize)
                validDirections = validDirections.Except(
                    new List<Direction>
                    { Direction.South, Direction.SouthWest, Direction.SouthEast});
        
            // If we're supposed to keep on moving in the same direction and it's valid, then we can exit
