    ...
    <data>
      <table name=""tbl_user"" entire=""Y"">
    	...
    	var data = serializer.Deserialize(new MemoryStream(System.Text.Encoding.UTF8.GetBytes(xml))) as Data;
    	Console.WriteLine($"{data.Table.Columns.Column.Count} columns");
    	Console.WriteLine($"{data.Table.Row.Count} rows");
        // Output
        // 3 columns
        // 4 rows
    }
