    using (SqlConnection sqliteCon = new SqlConnection(connection))
    {
        ...
        var query = "SELECT ResItem AS RD FROM tabStoreExec JOIN tabList ON tabStoreExec.idSE=tabList.idL WHERE tabList.Selection=1";
        ...
        int generatedId = 0;
        var query2 = "INSERT INTO tabStoricoDetail(NomeItem,ResItemDet,DateStartDet,DateEndDet) OUTPUT inserted.Id VALUES (@NI,@RProva,@DATESE,@DATEED)";
        ...
        var query3 = "UPDATE tabStoreExec SET FK_TSD_id = @tsdId FROM tabList JOIN tabStoreExec ON tabList.idL = tabStoreExec.idSE WHERE tabList.Selection=1 ";
