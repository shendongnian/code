        ...
        internal class Program
        {
            private static void Main(string[] args)
            {
                ...
                foreach (var kvp in Dict)
                {
                    ...
                    // for the current site, see what account/s has fewer responsibilities
                    int minResponsibilities = kvp.Value.Min(x => responsibilitiesCount[x]);
                    List<string> accounts = kvp.Value.Where(x => responsibilitiesCount[x] == minResponsibilities).ToList();
                    // for the resulted accounts, see witch has the fewer assigned jobs
                    int minAssignedJobs = accounts.Min(x => jobs.Count(y => y.Account == x));
                    string account = accounts.First(x => jobs.Count(y => y.Account == x) == minAssignedJobs);
