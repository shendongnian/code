    public class JavaStringComparer : IComparer<string>
    {
        public int Compare(string x, string y)
        {
            ...
            {                
                int result = x[i] - y[i];
                // Return the result of the first non-zero comparison, converting
                // the result to 1 or -1 by dividing it by it's absolute value
                if (result != 0) return result / Math.Abs(result);
            }
            // If the two strings have common beginnings, return the comparison of their lengths
