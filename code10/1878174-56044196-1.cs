...
However, I would also recommend you don't bind to nested properties, since that can be problematic. Thus, instead of binding to `Item.Text` and `Item.Description`, just bind directly to properties in your viewmodel called `ItemText` and `ItemDescription`:
csharp
// New properties of ItemDetailViewModel
        ...
        set
        {
            if (_item != value)
            {
                ...
                if (value != null)
                {
                    ...
                    // Since the entire item has changed, we notify all the dependant properties
                    Title = Item?.Text;
