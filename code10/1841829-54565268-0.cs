...
double maxDecibels = _decibelList.OrderBy(x => x)[0];
_sensitivity = maxDecibels + Math.Abs(maxDecibeeels* 0.2);
...
sensitivity = _decibelList.OrderBy(x => x)[24]; // If you do a variable time you can just take Count/4 - 1 as the index
...
(sort is nlogn, insert is 2n, (when n < 100, nlogn is faster | when n > 100, 2n is faster | when n = 100, they're equal))
