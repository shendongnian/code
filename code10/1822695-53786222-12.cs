    static class TimingInterfaceVsDelegateCalls
    {
        ...
        public static void Test()
        {
            ...
            Console.WriteLine($"Empty Work method: {emptyTime:n3}");
            stopWatch.Restart();
            for (int i = 0; i < N; i++) {
                double result = SomeWorkInterface(i);
            }
            stopWatch.Stop();
            Console.WriteLine($"Interface: {stopWatch.ElapsedMilliseconds * msToNs:n3} ({stopWatch.ElapsedMilliseconds * msToNs - emptyTime:n3})");
            stopWatch.Restart();
            for (int i = 0; i < N; i++) {
                double result = SomeWorkDelegate(i);
            }
            stopWatch.Stop();
            Console.WriteLine($"Delegates: {stopWatch.ElapsedMilliseconds * msToNs:n3} ({stopWatch.ElapsedMilliseconds * msToNs - emptyTime:n3})");
            stopWatch.Restart();
            for (int i = 0; i < N; i++) {
                double result = SomeWorkClass(i);
            }
            stopWatch.Stop();
            Console.WriteLine($"Class: {stopWatch.ElapsedMilliseconds * msToNs:n3} ({stopWatch.ElapsedMilliseconds * msToNs - emptyTime:n3})");
        }
