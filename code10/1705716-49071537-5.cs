    ...
    namespace ConsoleApp1
    {
        class Program
        {
            ...
            static void Main(string[] args)
            {
                ...
                Console.WriteLine($"{stopWatch.Elapsed.TotalMilliseconds} {expectedFail != null  && expectedFail.Status == IPStatus.Success}}");
                stopWatch = Stopwatch.StartNew();
                var expectedSuccess = WaitForActionCompletionOrTimeout(() => new Ping().Send("127.0.0.1", 200), 200).Result;
                Console.WriteLine($"{stopWatch.Elapsed.TotalMilliseconds} {expectedSuccess != null && expectedSuccess.Status == IPStatus.Success}");
