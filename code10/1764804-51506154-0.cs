    ...
    public static void Save(Game[] gA) {
        using (var f = File.Create(Directory.GetCurrentDirectory() + "\\sample.bin")) {
            new BinaryFormatter().Serialize(f, gA);
        }
    }
    
    public static Game[] Load() {
        using (var f = File.Open(Directory.GetCurrentDirectory() + "\\sample.bin", FileMode.Open)) {
            return (new BinaryFormatter().Deserialize(f)) as Game[];
    }
