...
class SimpleInjectorModelValidatorProvider : IModelValidatorProvider
{
    ...
    {
        var type = typeof(ModelValidator<>).MakeGenericType(ctx.ModelMetadata.ModelType);
        var validator = (IModelValidator)this.container.GetInstance(type);
...
container.Collection.Register(typeof(IValidator<>), typeof(MyViewmodelValidator).Assembly);
