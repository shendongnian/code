    ...
    public static void FixDatabarsAtWorksheet(OfficeOpenXml.ExcelWorksheet eworksheet)
    {
		...
		if (databars.Count > 0)
		{
			...
			{
				string temp_databar_id = CF.GenerateXlsId();
			...
			<x14:cfRule type=""dataBar"" id=""{" + temp_databar_id + @"}"">
				<x14:dataBar minLength=""" + (string.IsNullOrEmpty(left_val) ? "0" : left_val) + "\" maxLength=\"" + (string.IsNullOrEmpty(left_val) ? "100" : left_val) + "\" gradient=\"0\" " + (databar_border_color.IsEmpty ? string.Empty : "border = \"1\"") + ">";
