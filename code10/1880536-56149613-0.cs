    ...
    foreach (var range in List2)
    {
        ...
        {
            //result.Add(new Foo { StartTime = filteredDates[0], EndTime = filteredDates[^0] }); // in C# 8
            result.Add(new Foo { StartTime = filteredDates[0], EndTime = filteredDates[filteredDates.Length - 1] });
        }
        else
        {
