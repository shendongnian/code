       var orderByList = new List<Expression<Func<TEntity, object>>>();
    			...
    			var resultOrderedQueryable = orderByList.Aggregate<Expression<Func<TEntity, object>>, IOrderedQueryable<TEntity>>(null, (current, orderBy) => current != null ? current.ThenBy(x => x.Id) : query.OrderBy(orderBy));
