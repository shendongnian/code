    ...
    {
       public static bool IsAllDigit(this string[] x, int depth = 0) 
          => x.Length > depth && x[depth].All(char.IsDigit);
    
       public static bool IsAnyLetter(this string[] x, int depth = 0) 
          => x.Length > depth && x[depth].Any(char.IsLetter);
       ...
       public static IOrderedEnumerable<string[]> CustomThenBy(this IOrderedEnumerable<string[]> list,int depth =0)      
          => list.ThenByDescending(x => !x.IsAllDigit(depth))
                     .ThenByDescending(x => x.IsAnyLetter(depth))
                     .ThenBy(x => x.Elem(depth));
       
       public static IOrderedEnumerable<string[]> CustomOrderBy(this IEnumerable<string[]> list,int depth =0)  
          => list.OrderByDescending(x => !x.IsAllDigit(depth))
                     .ThenByDescending(x => x.IsAnyLetter(depth))
                     .ThenBy(x => x.Elem(depth));
