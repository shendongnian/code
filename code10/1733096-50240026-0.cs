    ...
    void BernsenThreshold()
    {
        ...
        for(int y=0; y<image.Height; y++)
        {
            for(int x=0; x<image.Width; x++)
            {
                ...
                pixel=color.R;
                ArrayList list=GetNeighboursSorted(x, y, image);
    ...
    static ArrayList GetNeighboursSorted(int x, int y, Bitmap image)
    {
        ...
        for(int i=i1; i<=i2; i++)
        {
            for(int j=j1; j<=j2; j++)
            {
                if(i!=y && j!=x) 
                {
                    list.Add(image.GetPixel(j, i).GetBrightness()*255);
                }
            }
        }
        list.Sort();
