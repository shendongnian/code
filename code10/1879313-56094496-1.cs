...
|----------------------------------------------|----------------|
|  4           |    2         |      1         | Camry/Scepter  |
As you can see, Camry is the relationship between the models and the company.
These models connect when VehicleId is 2 (from Camry) and ManufactId is 2 (from Vista and Scepter)
For Nissan instead:
|  9           |    null      |      9         | Nissan         |
|----------------------------------------------|----------------|
|  10          |    9         |      9         | Datsun         |
|----------------------------------------------|----------------|
|  11          |    9         |      9         | Datsun 13T     |
The data lacks from that relationship, so the **childInfo** variable is empty. It actually gets the VehicleId 10 from the first match where the ManufactId is 9 and then tries to find matches for the childInfo variable where the ManufactId is 10 (there are none). The same for the next one, 11.
...
One possible way to handle this (I've changed the variables for a better understanding):
....
