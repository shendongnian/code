...
This way, when you send message to Listener (TCP/MLLP server), it looks for Start Block in your incoming data. It never finds it. It just discards all your message considering garbage. Hence, you nothing get back from Listener.
With MLLP implemented, your message (the stuff you are writing on socket) should look like this:
    <VT>MSH|^~\\&|TestAppName||AVR||20181107201939.357+0000|<CR>|QRY^R02^QRY_R02|923456|P|2.5<FS><CR>
Note the `<VT>`, `<CR>` and `<FS>` are place holders in above message.
You may refer [this](https://saravanansubramanian.com/hl72xdotnetprogramming/) article for detailed information (Read step 4 and onward):
>     using System;
    using System.Net;
    ...
    namespace SimpleMllpHl7ClientAdvanced
    {
        public class Program
        {
            ...
            static void Main(string[] args)
            {
                ...
                testHl7MessageToTransmit.Append(START_OF_BLOCK)                    .Append("MSH|^~\\&|AcmeHIS|StJohn|CATH|StJohn|20061019172719||ORM^O01|MSGID12349876|P|2.3")
                    .Append(CARRIAGE_RETURN)
                    .Append("PID|||20301||Durden^Tyler^^^Mr.||19700312|M|||88 Punchward Dr.^^Los Angeles^CA^11221^USA|||||||")
                    .Append(CARRIAGE_RETURN)                    .Append("PV1||O|OP^^||||4652^Paulson^Robert|||OP|||||||||9|||||||||||||||||||||||||20061019172717|20061019172718")
                    .Append(CARRIAGE_RETURN)
...
For more open source code, you may refer [this](https://github.com/dib0/HL7Fuse) github project.
