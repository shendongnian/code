        ...
        void Login()
        {
            using (SqlConnection sqlConn = new SqlConnection(ConnectionString))
            {
                using (SqlCommand sqlComm = new SqlCommand("FROM tbl WHERE Username=@Username AND Password=@Password", sqlConn))
                {
                    ...
                    while (sqlRd.Read())
                    {
                        CurrentUser = new Controllers.User();
                        CurrentUser.FirstName = sqlRd["FirstName"].ToString();
                        CurrentUser.LastName = sqlRd["LastName"].ToString();
                        CurrentUser.MailAddress = sqlRd["MailAddress"].ToString();
