        ...
        public List<USBDriveInfo> GetUSBDrivesInfo(string UserName, string Password, string Domain)
        {
            ...
            using (ManagementObjectSearcher moSearcher = new ManagementObjectSearcher(mScope, mQuery, mOptions))
            {
                foreach (ManagementObject moDiskDrive in moSearcher.Get())
                {
                    ...
                    using (ManagementObjectSearcher moAssocPart = new ManagementObjectSearcher(moPartitionQuery))
                    {
                        foreach (ManagementObject moAssocPartition in moAssocPart.Get())
                        {
                            ...
                            using (ManagementObjectSearcher moLogDisk = new ManagementObjectSearcher(moLogDiskQuery))
                            {
