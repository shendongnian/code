    ...
    {
        public static DataTable ToDataTable(this KeyList keyList)
        {
            if (keyList == null)
                return null;
            ...
            var columnQuery = keyList.list_of_business_keys
                .SelectMany(k => k.business_key)
                .Select(p => new KeyValuePair<string, Type>(p.Key, p.Value == null ? typeof(object) : p.Value.GetType()))
                .Merge(p => p.Key, (p1, p2) => new KeyValuePair<string, Type>(p1.Key, MergeTypes(p1.Value, p2.Value)));
            foreach (var c in columnQuery)
                tbl.Columns.Add(c.Key, c.Value);
            foreach (var d in keyList.list_of_business_keys.Select(k => k.business_key))
            {
                var row = tbl.NewRow();
                row["ssid"] = keyList.ssid;
                row["dim"] = keyList.dim;
                row["type"] = keyList.type;
    ...
    {
        public static IEnumerable<TSource> Merge<TSource, TKey>(this IEnumerable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TSource, TSource> mergeSelector)
        {
