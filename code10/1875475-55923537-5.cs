...
The code behaves correctly, you iterate over `aArray.Where(x => x == "a")` which will, considering it's logic only be looped twice. In the first execution `i = 3` so `aArray[i] = "b";` meaning your array looks like this:
{ "a", "a", "a", "b" }
In the second loop iteration `i--` has now the value *2* and after executing `aArray[i] = "b";` your array is:
{ "a", "a", "b", "b" }
At this point, there are still `a`'s in your array, but the `LINQ` query returns `IEnumerator.MoveNext() == false` because it iterated over all matching items and the loop reaches its exit condition. This is the reaons why it _only prints out 2 a's and 2 b's, rather than 4 b's_.
> Why am I able to change what I'm looping over as I'm looping over it?
You are able to do so, not because your code is bad in any way, but because the build in code analyser in `Visual Studio` is not able to detect that you modify the collection within the loop. 
