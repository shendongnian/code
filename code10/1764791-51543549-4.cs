    ...
    namespace VisualStudioLikeControls
    {
        ...
        public partial class VSTabControl : TabControl
        {
            ...
            [Category("Custom"), Description("The color used for the components Alternative tab indicator color.  Set the Tabpages Tag property to 1, for each Tab you want it used on."), DefaultValue(typeof(Color), "OrangeRed")]
            ...
            protected override void OnMouseDown(MouseEventArgs e)
            {
                ...
                {
                    if (Alignment == TabAlignment.Top || Alignment == TabAlignment.Bottom) ptMaxDrag.X += GetTabRect(i).Width;
                    if (Alignment == TabAlignment.Top) { ptMaxDrag.Y = BorderSize; }
                    if (Alignment == TabAlignment.Bottom) { ptMaxDrag.Y = rectDrag.Y; };
    
                    if (Alignment == TabAlignment.Right || Alignment == TabAlignment.Left) { ptMaxDrag.X = rectDrag.X; ptMaxDrag.Y += GetTabRect(i).Height; }
                }
            }
